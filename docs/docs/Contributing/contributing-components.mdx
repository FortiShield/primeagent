...
New components are added as objects of the [`Component`](https://github.com/khulnasoft-bot/primeagent/blob/main/src/wfx/src/wfx/custom/custom_component/component.py) class.

Dependencies are added to the [pyproject.toml](https://github.com/khulnasoft-bot/primeagent/blob/main/pyproject.toml) file.

## Contribute an example component to Primeagent

Anyone can contribute an example component. For example, to create a new data component called **DataFrame processor**, follow these steps to contribute it to Primeagent.

<PartialBasicComponentStructure />

5. Save the `dataframe_processor.py` to the `src/wfx/src/wfx/components` directory.
   This example adds a data component, so add it to the `/data` directory.

6. Add the component dependency to `src/wfx/src/wfx/components/data/__init__.py` as `from .DataFrameProcessor import DataFrameProcessor`.
   You can view the [/data/**init**.py](https://github.com/khulnasoft-bot/primeagent/blob/dev/src/wfx/src/wfx/components/data/__init__.py) in the Primeagent repository.

7. Add any new dependencies to the [pyproject.toml](https://github.com/khulnasoft-bot/primeagent/blob/main/pyproject.toml#L20) file.

8. Submit documentation for your component. For this example component, you would submit documentation to the [Data components page](https://github.com/khulnasoft-bot/primeagent/blob/main/docs/docs/Components/components-data.mdx).

9. Submit your changes as a pull request. The Primeagent team will review, suggest changes, and add your component to Primeagent.
...
When documenting changes in pull requests, clearly explain _what_ changed, such as display name updates or new fields, _why_ it changed, such as improvements or bug fixes, and the _impact_ on existing users.
