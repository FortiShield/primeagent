---
title: Contribute to Primeagent
...
This guide is intended to help you start contributing to Primeagent.
As an open-source codebase in a rapidly developing field, Primeagent welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.

To contribute code or documentation to Primeagent, follow the [pull request guide](#open-a-pull-request).

## Primeagent services overview

This overview will help you understand how to set up your development environment.

Primeagent consists of two main services:
...
When you install Primeagent as an end user, the backend serves the frontend internally, making both services available on a single port (`7860` by default).

## Install Primeagent from source

Install Primeagent from source by forking the repository and setting up your development environment.
...
### Clone the Primeagent repository

1. Fork the [Primeagent GitHub repository](https://github.com/khulnasoft-bot/primeagent).

2. Add the new remote to your local repository on your local machine:

```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/primeagent.git
```
...
### Run Primeagent from source

You can run Primeagent from source after cloning the repository, even if you aren't contributing to the codebase.
This builds the frontend and serves it through the backend on port `7860`.

The instructions below are for running Primeagent from source. For development with hot reload, see [Set up development environment](#set-up-your-primeagent-development-environment).

<details>
<summary>Run from source on macOS/Linux</summary>

In your terminal, navigate to the root of the Primeagent directory, and then run `make run_cli`.
...
The Primeagent frontend is served at `http://localhost:7860`.
...
To run Primeagent from source on Windows, you can use the Primeagent repository's included scripts, or run the commands in the terminal.

Do one of the following:

- To install and run Primeagent with the included Windows Batch file, navigate to the `scripts/windows` directory, and then run the `build_and_run.bat` file.

- To run Primeagent from the Windows Command Line:
  ...
  2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/primeagent/frontend`.
...
- To install and run Primeagent with the included scripts, navigate to the `scripts/windows` directory, and then run the `build_and_run.ps1` file.

- To run Primeagent from a PowerShell terminal:
  1. Build the frontend static files:
...
### Set up your Primeagent development environment
...
If you just want to run Primeagent locally without making code changes, see [Run Primeagent from source](#run-primeagent-from-source).

<Tabs>
<TabItem value="linux-macos" label="Linux or macOS" default>

1. Set up the Primeagent development environment:
...
Since Windows doesn't include `make`, building and running Primeagent from source uses `npm` and `uv`.

To set up the Primeagent development environment, run the frontend and backend in separate terminals:
...
If you encounter frontend build problems or are upgrading from an older version of Primeagent, run `make run_clic` once.
...
2. Fork the [Primeagent GitHub repository](https://github.com/khulnasoft-bot/primeagent).
...
4. From the root of your local Primeagent fork, change to the `/docs` directory:
...
6. To edit and create content, work with the `.mdx` files in the `primeagent/docs/docs` directory.

   Create new files in `.mdx` format.

   Navigation is defined in `primeagent/docs/sidebars.js`.
...
- Pull request titles appear in Primeagent's release notes, so they should explain what the PR does as explicitly as possible.
- Pull requests should strive to fix one thing **only**, and should contain a good description of what is being fixed.

3. A Primeagent maintainer will review your pull request and may request changes, so ensure you pay attention to your PRs. Thanks for your contribution!
