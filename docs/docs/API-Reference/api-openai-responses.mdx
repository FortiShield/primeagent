...
Primeagent includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).
...
To be compatible with Primeagent's OpenAI Responses API endpoint, your flow and request must adhere to the following requirements:
...
Primeagent uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to Primeagent with OpenAI client libraries, you must configure custom headers and include an `api_key` configuration.
...
In the following examples, replace the values for `PRIMEAGENT_SERVER_URL`, `PRIMEAGENT_API_KEY`, and `FLOW_ID` with values from your deployment.
...
client = OpenAI(
  base_url="PRIMEAGENT_SERVER_URL/api/v1/",
  default_headers={"x-api-key": "PRIMEAGENT_API_KEY"},
  api_key="dummy-api-key" # Required by OpenAI SDK but not used by Primeagent
)
...
const client = new OpenAI({
  baseURL: 'PRIMEAGENT_SERVER_URL/api/v1/',
  defaultHeaders: {
...
const response = await client.responses.create({
  model: 'FLOW_ID',
  input:
    'There is an event that happens on the second wednesday of every month. What are the event dates in 2026?',
});
...
<details closed>
  ...
  ```text Here are the event dates for the second Wednesday of each month in
  2026: - January 14, 2026 - February 11, 2026 - March 11, 2026 - April 8, 2026
  - May 13, 2026 - June 10, 2026 - July 8, 2026 - August 12, 2026 - September 9,
  2026 - October 14, 2026 - November 11, 2026 - December 9, 2026 If you need
  these in a different format or want a downloadable calendar, let me know! ```
</details>
...
```bash
curl -X POST \
  "$PRIMEAGENT_SERVER_URL/api/v1/responses" \
...
| --------------------------- | -------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `x-api-key`                 | Yes      | Your Primeagent API key for authentication | `"sk-..."`                                                                                                               |
| `Content-Type`              | Yes      | Specifies the JSON format                  | `"application/json"`                                                                                                     |
| `X-PRIMEAGENT-GLOBAL-VAR-*` | No       | Global variables for the flow              | `"X-PRIMEAGENT-GLOBAL-VAR-API_KEY: sk-..."` For more, see [Pass global variables to your flows in headers](#global-var). |
...
The response contains fields that Primeagent sets dynamically and fields that use OpenAI-compatible defaults.
...
Fields set dynamically by Primeagent:
...
```bash
curl -X POST \
  "http://$PRIMEAGENT_SERVER_URL/api/v1/responses" \
  -H "x-api-key: $PRIMEAGENT_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "$FLOW_ID",
...
The `/responses` endpoint accepts global variables as custom HTTP headers with the format `X-PRIMEAGENT-GLOBAL-VAR-{VARIABLE_NAME}`.
...
This example demonstrates passing an `OPENAI_API_KEY` variable, which is a variable Primeagent automatically detects from environment variables, with two custom variables for `USER_ID` and `ENVIRONMENT`. The variables don't have to be created in Primeagent's Global Variables section - you can pass any variable name in the `X-PRIMEAGENT-GLOBAL-VAR-{VARIABLE_NAME}` header format.
...
curl -X POST \
  ...
  -H "x-api-key: $PRIMEAGENT_API_KEY" \
  -H "Content-Type: application/json" \
  -H "X-PRIMEAGENT-GLOBAL-VAR-OPENAI_API_KEY: sk-..." \
  -H "X-PRIMEAGENT-GLOBAL-VAR-USER_ID: user123" \
  -H "X-PRIMEAGENT-GLOBAL-VAR-ENVIRONMENT: production" \
  -d '{
    "model": "your-flow-id",
    "input": "Hello"
  }'
...
Variables passed with `X-PRIMEAGENT-GLOBAL-VAR-{VARIABLE_NAME}` are always available to your flow, regardless of whether they exist in the database.

If your flow components reference variables that aren't provided in headers or your Primeagent database, the flow fails by default.
