...
Retrieve logs for your Primeagent flows and server.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your Primeagent instance.

To enable log retrieval, set the following [environment variables](/environment-variables) in your Primeagent `.env` file, and then start Primeagent with `uv run primeagent run --env-file .env`:

```text
PRIMEAGENT_ENABLE_LOG_RETRIEVAL=True
PRIMEAGENT_LOG_RETRIEVER_BUFFER_SIZE=10000 // Must be greater than 0
PRIMEAGENT_LOG_LEVEL=DEBUG // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
...
```bash
curl -X GET \
  "$PRIMEAGENT_URL/logs?lines_before=0&lines_after=0&timestamp=0" \
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/primeagent/profile_pictures/Space.\n"
}
```
