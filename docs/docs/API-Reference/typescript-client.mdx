---
title: Primeagent TypeScript client
...
The Primeagent TypeScript client allows your TypeScript applications to programmatically interact with the Primeagent API.

For the client code repository, see [primeagent-client-ts](https://github.com/datastax/primeagent-client-ts/).

For the npm package, see [@datastax/primeagent-client](https://www.npmjs.com/package/@datastax/primeagent-client).

## Install the Primeagent TypeScript package

To install the Primeagent typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/primeagent-client
```
...
```bash
yarn add @datastax/primeagent-client
```
...
```bash
pnpm add @datastax/primeagent-client
```

</TabItem>
</Tabs>

## Initialize the Primeagent TypeScript client

1. Import the client into your code.

   ```tsx
   import { PrimeagentClient } from '@datastax/primeagent-client';
   ```

2. Initialize a `PrimeagentClient` object to interact with your server:

   ```tsx
   const baseUrl = 'BASE_URL';
   const apiKey = 'API_KEY';
   const client = new PrimeagentClient({ baseUrl, apiKey });
   ```

   Replace `BASE_URL` and `API_KEY` with values from your deployment.
   The default Primeagent base URL is `http://localhost:7860`.
   To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## Connect to your server and get responses

1. With your Primeagent client initialized, test the connection by calling your Primeagent server.
   ...
   const client = new PrimeagentClient({ baseUrl });
   ...
   - `baseUrl`: The URL of your Primeagent server.
...
2. Review the result to confirm that the client connected to your Primeagent server.

   The following example shows the response from a well-formed `runFlow` request that reached the Primeagent server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with Primeagent:
...
3.  Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming Primeagent responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve Primeagent logs with the TypeScript client

To retrieve [Primeagent logs](/logging), you must enable log retrieval on your Primeagent server by including the following values in your Primeagent `.env` file:

```text
PRIMEAGENT_ENABLE_LOG_RETRIEVAL=True
PRIMEAGENT_LOG_RETRIEVER_BUFFER_SIZE=10000
PRIMEAGENT_LOG_LEVEL=DEBUG
```
...
async function runFlow(client: PrimeagentClient) {
  const input = 'Is anyone there?';
...
async function main() {
  const client = new PrimeagentClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <primeagent.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
