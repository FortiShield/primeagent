...
Use the `/files` endpoints to move files between your local machine and Primeagent.

All `/files` endpoints (both `/v1/files` and `/v2/files`) require authentication with a Primeagent API key.
...
- `/v2` files are organized by `user_id` instead of `flow_id`.
  ...
  You can upload a file to Primeagent one time, and use it with multiple flows.
- `/v2` files are tracked in the Primeagent database.
...
```bash
curl -X POST \
  "$PRIMEAGENT_URL/api/v1/files/upload/$FLOW_ID" \
...
Send image files to Primeagent to use them in flows.

The default file limit is 1024 MB.
To change this limit, set the `PRIMEAGENT_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).

1. Attach the image to a `POST /v1/files/upload/$FLOW_ID` request with `--form` (`-F`) and the file path:

   ```bash
   curl -X POST "$PRIMEAGENT_URL/api/v1/files/upload/$FLOW_ID" \
     -H "Content-Type: multipart/form-data" \
     -H "x-api-key: $PRIMEAGENT_API_KEY" \
     -F "file=@PATH/TO/FILE.png"
   ```

   A successful request returns the `file_path` for the image in the Primeagent file management system in the format `FLOW_ID/TIMESTAMP_FILENAME.TYPE`.
   For example:
...
2. Use the returned `file_path` to send the image file to other components that can accept file input. Where you specify the file path depends on the component type.
   ...
   ```bash
   curl -X POST \
       "$PRIMEAGENT_URL/api/v1/run/a430cc57-06bb-4c11-be39-d3d4de68d2c4?stream=false" \
...
```bash
curl -X GET \
  "$PRIMEAGENT_URL/api/v1/files/list/$FLOW_ID" \
  -H "accept: application/json" \
  -H "x-api-key: $PRIMEAGENT_API_KEY"
```
...
```bash
curl -X GET \
  "$PRIMEAGENT_URL/api/v1/files/download/$FLOW_ID/2024-12-30_15-19-43_your_file.txt" \
  -H "accept: application/json" \
  -H "x-api-key: $PRIMEAGENT_API_KEY" \
...
```bash
curl -X DELETE \
  "$PRIMEAGENT_URL/api/v1/files/delete/$FLOW_ID/2024-12-30_15-19-43_your_file.txt" \
  -H "accept: application/json" \
  -H "x-api-key: $PRIMEAGENT_API_KEY"
```
...
To create a Primeagent API key and export it as an environment variable, see [Get started with the Primeagent API](/api-reference-api-examples).
...
1.  To retrieve your current `user_id`, call the `/whoami` endpoint:
    ...
    ```bash
    curl -X GET \
      "$PRIMEAGENT_URL/api/v1/users/whoami" \
      -H "accept: application/json" \
      -H "x-api-key: $PRIMEAGENT_API_KEY"
    ```
...
<summary>Result</summary>
    ...
    ```
    {"id":"07e5b864-e367-4f52-b647-a48035ae7e5e","username":"primeagent","profile_image":null,"store_api_key":null,"is_active":true,"is_superuser":true,"create_at":"2025-05-08T17:59:07.855965","updated_at":"2025-05-28T19:00:42.556460","last_login_at":"2025-05-28T19:00:42.554338","optins":{"github_starred":false,"dialog_dismissed":true,"discord_clicked":false,"mcp_dialog_dismissed":true}}
    ```
...
This endpoint uploads files to your Primeagent server's file management system.
To use an uploaded file in a flow, send the file path to a flow with a [**Read File** component](/read-file).

The default file limit is 1024 MB. To configure this value, change the `PRIMEAGENT_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).
...
2. To use this file in your flow, add a **Read File** component to your flow.
   This component loads files into flows from your local machine or Primeagent file management.

3. Run the flow, passing the `path` to the `Read-File` component in the `tweaks` object:

   ```text
   curl --request POST \
     --url "$PRIMEAGENT_URL/api/v1/run/$FLOW_ID" \
...
```bash
curl -X GET \
  "$PRIMEAGENT_URL/api/v2/files/c7b22c4c-d5e0-4ec9-af97-5d85b7657a34" \
  -H "accept: application/json" \
  -H "x-api-key: $PRIMEAGENT_API_KEY" \
  --output downloaded_file.txt
```
...
```bash
curl -X PUT \
  "$PRIMEAGENT_URL/api/v2/files/$FILE_ID?name=new_file_name" \
  -H "accept: application/json" \
  -H "x-api-key: $PRIMEAGENT_API_KEY"
```
...
```bash
curl -X DELETE \
  "$PRIMEAGENT_URL/api/v2/files/$FILE_ID" \
  -H "accept: application/json" \
  -H "x-api-key: $PRIMEAGENT_API_KEY"
```
