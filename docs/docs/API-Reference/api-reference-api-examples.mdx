---
title: Get started with the Primeagent API
...
You can use the Primeagent API for programmatic interactions with Primeagent, such as the following:

- Create and edit flows, including file management for flows.
- Develop applications that use your flows.
- Develop custom components.
- Build Primeagent as a dependency of a larger application, codebase, or service.
- Contribute to the overall Primeagent codebase.

To view and test all available endpoints, you can access the Primeagent API's OpenAPI specification at your Primeagent deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the Primeagent API in a script, see the [Primeagent quickstart](/get-started-quickstart).
...
## Form Primeagent API requests

While individual options vary by endpoint, all Primeagent API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a Primeagent API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

```bash
curl --request POST \
  --url "$PRIMEAGENT_SERVER_URL/api/v1/run/$FLOW_ID?stream=false" \
  --header "Content-Type: application/json" \
  --header "x-api-key: $PRIMEAGENT_API_KEY" \
  --data '{
...
By default, local deployments serve the Primeagent API at `http://localhost:7860/api`.

Remotely hosted Primeagent deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:PRIMEAGENT_PORT/api`.

You can configure the Primeagent port number in the `PRIMEAGENT_PORT` [environment variable](/environment-variables).

- `https://UUID.ngrok.app/api`
- `http://IP_OR_DNS/api`
- `http://IP_OR_DNS:PRIMEAGENT_PORT/api`

### Authentication

In Primeagent versions 1.5 and later, most API endpoints require authentication with a Primeagent API key in either an `x-api-key` header or query parameter.
...
Primeagent API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The Primeagent API serves `/v1` and `/v2` endpoints.
...
export PRIMEAGENT_SERVER_URL="https://localhost:7860"
...
Commonly used values in Primeagent API requests include your [Primeagent server URL](#base-url), [Primeagent API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some Primeagent API requests

Once you have your Primeagent server URL, try calling these endpoints that return Primeagent metadata.

### Health check

Returns the health status of the Primeagent database and chat services:

```bash
curl -X GET \
  "$PRIMEAGENT_SERVER_URL/health_check" \
...
Primeagent provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before Primeagent is fully initialized, so it's not reliable for checking Primeagent service health.

### Get version

Returns the current Primeagent API version:

```bash
curl -X GET \
  "$PRIMEAGENT_SERVER_URL/api/v1/version" \
  -H "accept: application/json"
```
...
Returns configuration details for your Primeagent deployment.
Requires a [Primeagent API key](/api-keys-and-authentication).

```bash
curl -X GET \
  "$PRIMEAGENT_SERVER_URL/api/v1/config" \
  -H "accept: application/json" \
...
Returns a dictionary of all Primeagent components.
Requires a [Primeagent API key](/api-keys-and-authentication).

```bash
curl -X GET \
  "$PRIMEAGENT_SERVER_URL/api/v1/all" \
  -H "accept: application/json" \
  -H "x-api-key: $PRIMEAGENT_API_KEY"
```
...
Because you can run Primeagent as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the Primeagent database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the Primeagent codebase, you won't directly call most of the Primeagent endpoints.
...
The following endpoints are useful for developing applications with Primeagent and administering Primeagent deployments with one or more users.
...
- Deployment details:
  - GET `/v1/version`: Return Primeagent version. See [Get version](/api-reference-api-examples#get-version).
...
- Custom components: You might use these endpoints when developing custom Primeagent components for your own use or to share with the Primeagent community:
  - GET `/v1/all`: Return all available Primeagent component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the Primeagent codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
- MCP servers: The following endpoints are for managing Primeagent MCP servers and MCP server connections.
  They aren't typically called directly; instead, they are used to drive internal functionality in the Primeagent frontend and when running flows that call MCP servers.
  Primeagent MCP servers support both streamable HTTP and SSE transport.
...
- Use the Primeagent API to [run a flow](/api-flows-run).
- Use the Primeagent API to [upload files](/api-files).
- Use the Primeagent API to [get flow logs](/api-logs).
- Explore all endpoints in the [Primeagent API specification](/api).
