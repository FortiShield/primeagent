---
title: Deploy Primeagent with Nginx and SSL
slug: /deployment-nginx-ssl
---

import Icon from '@site/src/components/icon';

Deploy Primeagent on a Linux-based server using Nginx as a reverse proxy, Let's Encrypt for SSL certificates, and Certbot for automated certificate management.

This setup encrypts all communications between users and your Primeagent server. SSL certificates ensure that sensitive data is protected from eavesdropping and tampering, and the automatic certificate management through Certbot eliminates the complexity of manual SSL configuration.
...
## Install Primeagent on your server

To install Primeagent on your server, do the following:
...
3. Install uv to manage Python packages.
   Primeagent recommends uv for faster installation.

   ```bash
   pip install uv
   ```

4. Create a virtual environment for Primeagent.

   ```bash
   uv venv primeagent-venv
   source primeagent-venv/bin/activate
   ```

5. Install Primeagent using uv.

   ```bash
   uv pip install primeagent
   ```

6. Optionally, start Primeagent.

   ```bash
   uv run primeagent run --host 127.0.0.1 --port 7860 &
   ```

   Test access at your `http://YOUR_PUBLIC_IP:7860`.
   Since Primeagent is running on `localhost`, you should not be successful.
...
Nginx is a reverse proxy that receives external requests and forwards them to your Primeagent server. In addition to SSL termination, Nginx includes features for load balancing and security. For more information, see the [Nginx documentation](https://nginx.org/en/docs/).
...
3. Create an Nginx configuration file.
   Replace **DOMAIN_NAME** with your actual domain name, such as `primeagent.example.com`.
   `bash
...
4. Paste the following content to the configuration file you created at `/etc/nginx/sites-available/DOMAIN_NAME`.
   ...
   server {
       ...
       location / {
           ...
           # WebSocket support for Primeagent
...
3. Start Primeagent from the virtual environment.
   ...
   b. Start Primeagent in the background.
...
- [Deploy Primeagent on a remote server with Caddy](/deployment-caddyfile)
