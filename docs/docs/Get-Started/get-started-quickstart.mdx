...
Get started with Primeagent by loading a template flow, running it, and then serving it at the `/run` API endpoint.
...
- Create a [Primeagent API key](/api-keys-and-authentication)
    ...
    <details>
  <summary>Create a Primeagent API key</summary>

  A Primeagent API key is a user-specific token you can use with Primeagent.

  To create a Primeagent API key, do the following:
  1.  In Primeagent, click your user icon, and then select **Settings**.
  2.  Click **Primeagent API Keys**, and then click <Icon name="Plus" aria-hidden="true"/> **Add New**.
  3.  Name your key, and then click **Create API Key**.
  4.  Copy the API key and store it securely.
  5.  To use your Primeagent API key in a request, set a `PRIMEAGENT_API_KEY` environment variable in your terminal, and then include an `x-api-key` header or query parameter with your request.
      For example:
          ...
          # Send request
          curl --request POST \
          --url "http://PRIMEAGENT_SERVER_ADDRESS/api/v1/run/FLOW_ID" \
...
Primeagent is an IDE, but it's also a runtime you can call through the [Primeagent API](/api-reference-api-examples) with Python, JavaScript, or HTTP.

When you start Primeagent locally, you can send requests to the local Primeagent server.
For production applications, you need to [deploy a stable Primeagent instance](/deployment-overview) to handle API calls.

For example, you can use the `/run` endpoint to run a flow and get the result.

Primeagent provides code snippets to help you get started with the Primeagent API.

1. When editing a flow, click **Share**, and then click **API access**.

   The default code in the API access pane constructs a request with the Primeagent server `url`, `headers`, and a `payload` of request data.
   The code snippets automatically include the `PRIMEAGENT_SERVER_ADDRESS` and `FLOW_ID` values for the flow, and a script to include your `PRIMEAGENT_API_KEY` if you've set it as an environment variable in your terminal session.
...
<TabItem value="Python" label="Python" default>
   ...
   url = "http://PRIMEAGENT_SERVER_ADDRESS/api/v1/run/FLOW_ID"  # The complete API endpoint URL for this flow
...
<TabItem value="JavaScript" label="JavaScript">
   ...
   fetch('http://PRIMEAGENT_SERVER_ADDRESS/api/v1/run/FLOW_ID', options)
...
<TabItem value="curl" label="curl">
   ...
   ```bash
   curl --request POST \
        --url 'http://PRIMEAGENT_SERVER_ADDRESS/api/v1/run/FLOW_ID?stream=false' \
        --header 'Content-Type: application/json' \
        --header "x-api-key: PRIMEAGENT_API_KEY" \
        --data '{
         "output_type": "chat",
...
In a production application, you probably want to select parts of this response to return to the user, store in logs, and so on. The next steps demonstrate how you can extract data from a Primeagent API response to use in your application.
...
1.  Incorporate your **Simple Agent** flow's `/run` snippet into the following script.
    This script runs a question-and-answer chat in your terminal and stores the agent's previous answer so you can compare them.
        ...
        const url = 'http://PRIMEAGENT_SERVER_ADDRESS/api/v1/run/FLOW_ID';
...
To assist with formatting, you can define tweaks in Primeagent's **Input Schema** pane before copying the code snippet.
...
3. For example, to change the LLM provider from OpenAI to Groq, and include your Groq API key with the request, select the values **Model Providers**, **Model**, and **Groq API Key**.
   Primeagent updates the `tweaks` object in the code snippets based on your input parameters, and includes default values to guide you.
...
- [Trigger flows with the Primeagent API](/concepts-publish)
- [Use Primeagent as a Model Context Protocol (MCP) server](/mcp-server)
