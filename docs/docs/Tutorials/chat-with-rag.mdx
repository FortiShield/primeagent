...
This tutorial demonstrates how you can use Primeagent to create a chatbot application that uses Retrieval Augmented Generation (RAG) to embed your data as vectors in a vector database, and then chat with the data.
...
- Install the [Primeagent JavaScript client](/typescript-client)
- Be familiar with vector search concepts and applications, such as vector databases and RAG

## Create a vector RAG flow

1.  In Primeagent, click **New Flow**, and then select the **Vector Store RAG** template.
...
In situations where many users load data or you need to load data programmatically, use the Primeagent API option.
...
2. Select the local file you want to upload, and then click **Open**.
   The file is loaded to your Primeagent server.
...
<TabItem value="API" label="Primeagent API">

To load data programmatically, use the `/v2/files/` and `/v1/run/$FLOW_ID` endpoints. The first endpoint loads a file to your Primeagent server, and then returns an uploaded file path. The second endpoint runs the **Load Data Flow**, referencing the uploaded file path, to chunk, embed, and load the data into the vector store.

The following script demonstrates this process.
For help with creating this script, use the [Primeagent File Upload Utility](https://primeagent-file-upload-examples.21cgwws14kdz.us-east.codeengine.appdomain.cloud/).
...
const headers = { 'x-api-key': 'PRIMEAGENT_API_KEY' };

// 2. Upload the file to Primeagent
const uploadRes = await fetch('PRIMEAGENT_SERVER_ADDRESS/api/v2/files/', {
  method: 'POST',
  headers,
  body: data,
});
...
// 3. Call the Primeagent run endpoint with the uploaded file path
const payload = {
...
const runRes = await fetch('PRIMEAGENT_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: {
...
const primeagentData = await runRes.json();
// Output only the message
console.log(
  primeagentData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text,
);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
   ...
   const { PrimeagentClient } = require('@datastax/primeagent-client');

   # pragma: allowlist nextline secret
   const API_KEY = 'PRIMEAGENT_API_KEY';
   const SERVER = 'PRIMEAGENT_SERVER_ADDRESS';
   const FLOW_ID = 'FLOW_ID';

   const rl = readline.createInterface({ input: process.stdin, output: process.stdout });

   // Initialize the Primeagent client
   const client = new PrimeagentClient({
       baseUrl: SERVER,
       apiKey: API_KEY
   });
   ...
   function chat() {
       console.log('🤖 Primeagent RAG Chatbot (type "quit" to exit)\n');
   ...
   :::tip
   The [Primeagent TypeScript client](/typescript-client) has a `chatOutputText()` convenience method that simplifies working with Primeagent's complex JSON response structure.
