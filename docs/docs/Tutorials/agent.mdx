...
This tutorial shows you how to connect a JavaScript application to a [Primeagent agent](/agents).
...
In this example, the application sends a customer's email address to the Primeagent agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
2.  Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step.
    ...
    If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
        ...
        const PRIMEAGENT_SERVER_ADDRESS = 'PRIMEAGENT_SERVER_ADDRESS';
        const FLOW_ID = 'FLOW_ID';
        const PRIMEAGENT_API_KEY = 'PRIMEAGENT_API_KEY';
        ...
        async function runAgentFlow(): Promise<void> {
            try {
                ...
                const client = new PrimeagentClient({
                    baseUrl: PRIMEAGENT_SERVER_ADDRESS,
                    apiKey: PRIMEAGENT_API_KEY
                });

                console.log(`Connecting to Primeagent server at: ${PRIMEAGENT_SERVER_ADDRESS} `);
                ...
                if (error instanceof Error) {
                    if (error.message.includes('fetch')) {
                        console.error('\nMake sure your Primeagent server is running and accessible at:', PRIMEAGENT_SERVER_ADDRESS);
                    }
        ...
        // Run the function
        console.log('Starting Primeagent Agent...\n');
...
<summary>Result</summary>
    ...
    ```
    Starting Primeagent Agent...

    Connecting to Primeagent server at: http://localhost:7860
