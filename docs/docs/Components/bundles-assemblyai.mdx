...
- An [AssemblyAI account](https://www.assemblyai.com/dashboard/signup) and an AssemblyAI API key.
  ...
  Enter the key in the _AssemblyAI API Key_ field in all Primeagent components that require the AssemblyAI key.
...
- **Input**:
  ...
  - Speech Model (Optional): Select the class of models. Default is _Best_. See [speech models](https://www.assemblyai.com/docs/speech-to-text/speech-recognition#select-the-speech-model-with-best-and-nano) for more info.
...
1. Build the flow manually or import a pre-build JSON file:
   - Recommended: [Download the AssemblyAI Transcription and Speech AI flow JSON](/files/AssemblyAI_Flow.json), and then [import the flow](/concepts-flows-import) into Primeagent.
