...
Bundles contain custom components that support specific third-party integrations with Primeagent.
You add them to your flows and configure them in the same way as Primeagent's core components.
...
Many bundled components are developed by third-party contributors to the Primeagent codebase.
...
To find documentation for a specific bundled component, browse the Primeagent docs and your provider's documentation.
...
The Primeagent documentation focuses on using bundles within flows.
For that reason, it focuses on the Primeagent-specific configuration steps for bundled components.
...
:::tip
The Primeagent documentation doesn't list all bundles or components in bundles.
For the most accurate and up-to-date list of bundles and components for your version of Primeagent, check <Icon name="Blocks" aria-hidden="true" /> **Bundles** in the visual editor.

If you can't find a component that you used in an earlier version of Primeagent, it may have been removed or marked as a [legacy component](#legacy-bundles).
:::

Primeagent offers generic <Icon name="Component" aria-hidden="true" /> **Core components** in addition to third-party, provider-specific bundles.
...
The **Local DB** component reads and writes to a persistent, in-memory Chroma DB instance intended for use with Primeagent.
It has separate modes for reads and writes, automatic collection management, and default persistence in your Primeagent cache directory.
...
<!-- Not documented but in Primeagent as of 1.5.11 -->
<!--
* AgentQL
* Confluence
* Firecrawl
