...
:::tip
Primeagent provides keyboard shortcuts for the workspace.

In the Primeagent header, click your profile icon, select **Settings**, and then click **Shortcuts** to view the available shortcuts.
:::
...
- <Icon name="Component" aria-hidden="true" /> **Core components**: Primeagent's
  base components are grouped by purpose, such as **Inputs and Outputs** or
  **Data**. These components either provide generic functionality, like loops
  and parsing, or they provide single components that support multiple
  third-party integrations.

- <Icon name="Blocks" aria-hidden="true" /> **Bundles**: Bundles contain one or
  more components that support specific third-party integrations, and they are
...
For information about the programmatic representation of each data type, see [Primeagent data types](/data-types).
...
For example, the [**Recursive Character Text Splitter** component](https://github.com/khulnasoft-bot/primeagent/blob/main/src/wfx/src/wfx/components/langchain_utilities/recursive_character.py) is a child of the [`LCTextSplitterComponent`](https://github.com/khulnasoft-bot/primeagent/blob/main/src/wfx/src/wfx/base/textsplitters/model.py) class.
...
Component versions and states are stored in an internal Primeagent database. When you add a component to a flow, you create a detached copy of the component based on the information in the Primeagent database.
These copies are detached from the primary Primeagent database, and they don't synchronize with any updates that can occur when you upgrade your Primeagent version.
...
When editing a flow in the workspace, Primeagent notifies you if a component's workspace version is behind the database version so you can update the component's workspace version:
...
Components are updated to the latest available version, based on the version of Primeagent you are running.
