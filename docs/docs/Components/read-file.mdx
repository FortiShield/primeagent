...
In Primeagent version 1.7.0, this component was renamed from **File** to **Read File**.
...
The **Read File** component can read files from the local Primeagent database, **AWS S3**, or **Google Drive**.
...
If you need to load an unsupported file type, you must use a different component that supports that file type and, potentially, parses it outside Primeagent, or you must convert it to a supported type before uploading it.
...
| ----------------------------------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| path                                | Files                               | Input parameter. The path to files to load. Can be local or in [Primeagent file management](/concepts-file-management). Supports individual files and bundled archives.                                                                                   |
| file_path                           | Server File Path                    | Input parameter. A `Data` object with a `file_path` property pointing to a file in [Primeagent file management](/concepts-file-management) or a `Message` object with a path to the file. Supersedes **Files** (`path`) but supports the same file types. |
...
Starting in Primeagent version 1.6, the **Read File** component supports advanced document parsing using the [Docling](https://docling-project.github.io/docling/) library for supported file types.

To use advanced parsing, do the following:

1.  Complete the following prerequisites, if applicable:
    - **Install Primeagent version 1.6 or later**: Earlier versions don't support advanced parsing with the **Read File** component. For upgrade guidance, see the [Release notes](/release-notes).
    ...
    - **Enable Developer Mode for Windows**:
      ...
      <PartialDevModeWindows />
          ...
          Developer Mode isn't required for Primeagent OSS on Windows.
...
- **File Path**: A [`Message`](/data-types#message) containing the path to the file in [Primeagent file management](/concepts-file-management).
