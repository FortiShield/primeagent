...
You can use the **NVIDIA** component to connect Primeagent with NVIDIA NIM on an RTX Windows system with [Windows Subsystem for Linux 2 (WSL2)](https://learn.microsoft.com/en-us/windows/wsl/install) installed.
The following example connects an **NVIDIA** language model component in Primeagent to a deployed `mistral-nemo-12b-instruct` NIM on an **RTX Windows system** with **WSL2**.
...
1. Prepare your system:
   ...
   - The **NVIDIA Retriever Extraction** component requires the installation of additional dependencies to your Primeagent environment. To install the dependencies in a virtual environment, run the following commands.
     - If you have the Primeagent repository cloned and installed from source:
     ...
     - If you are installing Primeagent from the Python Package Index:

     ```bash
     source **YOUR_PRIMEAGENT_VENV**/bin/activate
     uv pip install --prerelease=allow 'primeagent[nv-ingest]'
     uv run primeagent run
     ```
...
1.  Prepare your system:
    ...
    - It uses the `gassist.rise` package, which is installed with all Primeagent versions that include this component.
