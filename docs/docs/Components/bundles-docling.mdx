...
Primeagent integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing and chunking documents.
...
- **Install Docling dependency**:
  The Docling dependency is required to use the Docling components in Primeagent.

      * **Primeagent version 1.6 and later**: The Docling dependency is included by default for all operating systems except macOS Intel (x86_64).
      ...
      * **Earlier versions**: Primeagent versions earlier than 1.6 don't include the Docling dependency.
      For Primeagent OSS, install the Docling extra with `uv pip install 'primeagent[docling]'`.
      For Primeagent Desktop, add the Docling dependency to Primeagent Desktop's `requirements.txt`.
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + Primeagent: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
