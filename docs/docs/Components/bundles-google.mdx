...
<Icon name="Blocks" aria-hidden="true" /> [**Bundles**](/components-bundle-components) contain custom components that support specific third-party integrations with Primeagent.

This page describes the components that are available in the **Google** bundle.

## BigQuery

Primeagent integrates with [Google BigQuery](https://cloud.google.com/bigquery) through the **BigQuery** component in the [**Google** bundle](/bundles-google), allowing you to execute SQL queries and retrieve data from your BigQuery datasets.
...
* A [running Primeagent server](/get-started-installation)
...
This role allows Primeagent to access BigQuery resources with the service account.
...
Now that you have a service account and a JSON private key, you need to configure the credentials in the Primeagent **BigQuery** component.

#### Configure credentials in the Primeagent component

With your service account configured and your credentials JSON file created, follow these steps to authenticate the Primeagent application.

1. Create a new flow in Primeagent.
...
2. Open the **Playground**, and then submit a valid SQL query.
    ...
    FROM `big-query-primeagent-project.the_oscar_award.oscar_winners`
    WHERE winner = TRUE
    LIMIT 10
    ```
...
Primeagent includes multiple components that support Google Search, such as the following:
...
The **Google OAuth Token** component was deprecated in Primeagent 1.4.0.
