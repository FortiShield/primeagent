...
To enable Arize tracing, set the required Arize environment variables in your Primeagent deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating Primeagent and Arize are also available in the Arize documentation:

- [Primeagent tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/primeagent/primeagent-tracing)
- [Primeagent tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/primeagent/primeagent-tracing)
  :::
...
2. In the root of your Primeagent application, edit your existing Primeagent `.env` file or create a new one.
...
4. Start your Primeagent application with your `.env` file:
...
1.  In Primeagent, run a flow that has an LLM-driven component, such as an **Agent** component or any language model component.
...
3.  To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each Primeagent execution generates two traces in Arize:
    - The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    - The `UUID` trace is the trace of the Primeagent components.
