...
description: Instrument Primeagent with the Traceloop SDK, and export traces and metrics to Instana using OpenTelemetry.
---
...
This guide demonstrates how to integrate the Instana observability platform with your Primeagent application using the Traceloop SDK so you can monitor and analyze LLM performance.
...
1. In the root folder of your Primeagent application, edit your existing Primeagent `.env` file or create a new one.
...
## Start Primeagent with Traceloop environment variables

Launch your Primeagent application with your `.env` file:
...
1. Run a flow in Primeagent to generate traffic.
2. To view traces in Instana, open Instana and click **Applications**.
3. In **Services**, search for `Primeagent`.
4. Click **Primeagent** to view and analyze the associated calls.
