...
:::info
Voice mode is not available in Primeagent Desktop.
To use voice mode, [Install the Primeagent OSS Python package](/get-started-installation#install-and-run-the-primeagent-oss-python-package).
:::

You can use Primeagent's voice mode to interact with your flows verbally through a microphone and speakers.
...
- An [OpenAI](https://platform.openai.com/) account and an OpenAI API key because Primeagent uses the OpenAI API to process voice input and generate responses.
...
5. Enter your OpenAI API key, and then click **Save**. Primeagent saves the key as a [global variable](/configuration-global-variables).
...
8. Optional: Add an ElevenLabs API key to enable more voices for the LLM's response.
   Primeagent saves this key as a global variable.
...
Primeagent exposes two OpenAI Realtime API-compatible websocket endpoints for your flows.
...
The Primeagent API's websockets endpoints require an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and they support an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
...
This is the mode used in the Primeagent **Playground**.
...
If omitted, Primeagent uses the flow ID as the [session ID](/session-id).
