...
In the Primeagent **Playground**, current sessions are listed on the left side of the pane.

Primeagent uses session IDs to track different chat interactions within flows. This allows multiple chat sessions to exist in a single flow. Messages are stored in the database with session IDs as a reference.
...
Replace `PRIMEAGENT_SERVER_ADDRESS`, `FLOW_ID`, and `PRIMEAGENT_API_KEY` with the values from your Primeagent deployment.
```
curl --request POST \
...
The `my_custom_session_value` value is used in components that accept it, and the stored messages from this flow are stored in `primeagent.db` with their respective `session_id` values.

## Retrieval of messages from memory by session ID

To retrieve messages from local Primeagent memory, add a [**Message History** component](/message-history) to your flow.
...
Messages can be retrieved by `session_id` from the Primeagent API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://docs.agent.khulnasoft.com/api-monitor).

For an example of session ID in action, see [Use Session IDs in Primeagent](https://www.youtube.com/watch?v=nJiF_eF21MY).