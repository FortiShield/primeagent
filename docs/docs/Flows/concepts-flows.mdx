...
Primeagent flows are fully serializable and can be saved and loaded from the file system where Primeagent is installed.

:::tip
To try building and running a flow in a few minutes, see the [Primeagent quickstart](/get-started-quickstart).
:::
...
- **Create a flow from a template**: Select a project, click **New Flow**, and then click the template that you want to use.
  ...
  You can also [contribute templates](/contributing-templates) to the Primeagent codebase.
...
You can also create a flow with the [Primeagent API](/api-flows), but the Primeagent team recommends using the [visual editor](/concepts-overview) until you are familiar with flow creation.
...
When you're ready to use Primeagent for application development, learn how to [trigger flows with the Primeagent API](/concepts-publish), explore more advanced configuration options like [custom dependencies](/install-custom-dependencies), and, eventually, [containerize your Primeagent application](/develop-application).

When you're ready to go to production or deploy a Primeagent MCP server for access over the public internet, see [Primeagent deployment overview](/deployment-overview).

#### Flow graphs

When a flow runs, Primeagent builds a Directed Acyclic Graph (DAG) object from the nodes (components) and edges (connections), and the nodes are sorted to determine the order of execution.
...
The **Projects** page is where you arrive when you launch Primeagent.
From here, you can manage flows and your projects' [MCP servers](/mcp-server).

Primeagent projects are like folders that you can use to organize related flows.
...
:::tip
To get back to the **Projects** page after editing a flow, click the project name or Primeagent icon in the Primeagent header.
:::
...
By default, flows and flow execution data are stored in the Primeagent database, and flow logs are stored with other Primeagent logs in the Primeagent config directory.
