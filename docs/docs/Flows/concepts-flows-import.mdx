...
You can export flows to transfer them between Primeagent instances, share them with others, or create backups.
...
- **Export with the Primeagent API**: To export one flow, use the [`/flows/download`](/api-flows#export-flows) endpoint.
...
For more information, see [Primeagent JSON file contents](#primeagent-json-file-contents).
...
If your key is stored in a Primeagent global variable, **Save with my API keys** exports only the variable name.
:::

When you or another user import the flow to another Primeagent instance, that instance must have Primeagent global variables with the same names and valid values in order to run the flow successfully.
If any variables are missing or invalid, those variables must be created or edited after importing the flow.

## Import a flow

You can import Primeagent JSON files from your local machine in the following ways:

- **Import to projects**: On the **Projects** page, click <Icon name="Upload" aria-hidden="true"/> **Upload a flow**, and then select the Primeagent JSON file to import.
- **Import anywhere**: Drag and drop Primeagent JSON files from your file explorer into your Primeagent window to import a flow from any Primeagent page.
- **Import with the Primeagent API**: To import one Primeagent JSON file, use the [`/flows/upload/`](/api-flows#import-flows) endpoint.
  To import a zip archive of Primeagent JSON files, use the [`/projects/upload`](/api-projects#import-a-project) endpoint.

### Run an imported flow

Once imported, your flow is ready to use.
If the flow contains any global variables, make sure your Primeagent instance has global variables with the same names and valid values.
For more information, see [Save with my API keys](/concepts-flows-import#save-with-my-api-keys).

## Primeagent JSON file contents

An exported flow is downloaded to your local machine as a JSON file named `FLOW_NAME.json`.

Primeagent JSON files contain the following:
...
For example Primeagent JSON files, you can examine any of the https://github.com/khulnasoft-bot/primeagent/tree/main/src/backend/base/primeagent/initial_setup/starter_projects[templates in the Primeagent repository], or you can create a flow from a template in Primeagent, export it, and then open the exported JSON file in a text editor.
...
- <Icon name="StickyNote" aria-hidden="true" /> **Notes** are comments that help
  explain the flow's purpose, configuration details, and any other information
  relevant to users who might be editing the flow. They can contain text, links,
  code snippets, and other information. They are encoded in Markdown format and
  stored as `node` objects.
