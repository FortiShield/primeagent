...
def warn_about_future_cors_changes(settings):
    ...
    if using_defaults:
        logger.warning(
            ...
            "Set PRIMEAGENT_CORS_ORIGINS for production. Stricter defaults in v2.0."
        )
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            await logger.adebug(f"Starting Primeagent v{version}...")
        else:
            await logger.adebug("Starting Primeagent...")
        ...
        try:
            ...
            lock_file = Path(tempfile.gettempdir()) / "primeagent_starter_projects.lock"
            ...
            from primeagent.api.v1.mcp import start_streamable_http_manager
            from primeagent.api.v1.mcp_projects import start_project_task_group
            ...
            if "primeagent migration --fix" not in str(exc):
            ...
            from primeagent.__main__ import get_number_of_workers
            from primeagent.cli.progress import create_primeagent_shutdown_progress

            log_level = os.getenv("PRIMEAGENT_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_primeagent_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(1):
                    from primeagent.api.v1.mcp import stop_streamable_http_manager
                    from primeagent.api.v1.mcp_projects import stop_project_task_group
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("Primeagent shutdown complete")
...
def create_app():
    ...
    if prome_port_str := os.environ.get("PRIMEAGENT_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from primeagent.api.v1 import mcp_router
...
def get_static_files_dir():
    """Get the static files directory relative to Primeagent's main.py file."""
