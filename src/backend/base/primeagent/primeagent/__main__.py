...
from primeagent.services.auth.utils import get_current_user_from_access_token
...
# Add WFX commands as a sub-app
try:
    from wfx.cli.commands import serve_command
    from wfx.cli.run import run as wfx_run

    wfx_app = typer.Typer(name="wfx", help="Primeagent Executor commands")
    wfx_app.command(name="serve", help="Serve a flow as an API", no_args_is_help=True)(serve_command)
    wfx_app.command(name="run", help="Run a flow directly", no_args_is_help=True)(wfx_run)

    app.add_typer(wfx_app, name="wfx")
except ImportError:
    # WFX not available, skip adding the sub-app
    pass
...
def run(
    *,
    ...
    log_level = (log_level or os.environ.get("PRIMEAGENT_LOG_LEVEL") or "info").lower()
    os.environ["PRIMEAGENT_LOG_LEVEL"] = log_level
    ...
    progress = create_primeagent_progress(verbose=verbose)

    # Step 0: Initializing Primeagent
    ...
    with progress.step(1):
        for key, value in os.environ.items():
            new_key = key.replace("PRIMEAGENT_", "")
    ...
    if platform.system() == "Windows":
        ...
        with progress.step(6):
            # Use Gunicorn with PrimeagentUvicornWorker for non-Windows systems
            from primeagent.server import PrimeagentApplication
            ...
            server = PrimeagentApplication(app, options)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "primeagent")
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use primeagent.utils.version to get the version info
    version_info = get_version_info()
    primeagent_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= primeagent_is_pre_release(primeagent_version)  # Update pre-release status

    notice = build_version_notice(primeagent_version, package_name)
    ...
    telemetry_text = (
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("PRIMEAGENT_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve Primeagent.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open Primeagent {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set PRIMEAGENT_ENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from primeagent.services.database.models.user.crud import get_all_superusers
    ...
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            ...
            typer.echo("1. Set PRIMEAGENT_AUTO_LOGIN=false")
    ...
    # AUTO_LOGIN is false - production mode
        ...
        if not auth_token:
            ...
            typer.echo("To get a token, use: `uv run primeagent api_key`")
            raise typer.Exit(1)
    ...
    async with session_scope() as session:
        from primeagent.services.deps import get_auth_service
...
# command to copy the primeagent database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'primeagent.db' and 'primeagent-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "primeagent.db"
    pre_db_path = cache_dir / "primeagent-pre.db"
...
def show_version(*, value: bool):
    if value:
        ...
        typer.echo(f"primeagent {version}")
        raise typer.Exit
