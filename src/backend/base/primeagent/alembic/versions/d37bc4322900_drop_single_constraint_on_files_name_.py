...
def upgrade() -> None:
    ...
    logger.info("Running migration on %s database", db_dialect)
        ...
        if db_dialect == "sqlite":
            ...
            if constraints_to_drop:
                for constraint_name in constraints_to_drop:
                    ...
                    logger.info("PostgreSQL: Dropped constraint %s", constraint_name)
            else:
            ...
            for (index_name,) in idx_result.fetchall():
                ...
                logger.info("PostgreSQL: Dropped unique index %s", index_name)

        else:
            error_msg = f"Unsupported database dialect: {db_dialect}"
            raise ValueError(error_msg)

    except Exception:
        logger.exception("Error during constraint removal")
        raise
...
def downgrade() -> None:
    ...
    try:
        ...
        if dup:
            error_msg = (
                "Downgrade aborted: duplicates in file.name would violate UNIQUE(name). Deduplicate before downgrading."
            )
            raise RuntimeError(error_msg)
        if db_dialect == "sqlite":
            ...
            expected = ["id", "user_id", "name", "path", "size", "provider", "created_at", "updated_at"]
            if set(cols) != set(expected):
                error_msg = f"SQLite: Unexpected columns on file table: {cols}. Aborting downgrade."
                raise RuntimeError(error_msg)
            ...
            logger.info("Downgrade not supported for dialect: %s", db_dialect)

    except Exception as e:
        logger.exception("Error during downgrade")
