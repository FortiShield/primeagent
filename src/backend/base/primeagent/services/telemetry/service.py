...
class TelemetryService(Service):
    ...
    def _get_primeagent_desktop(self) -> bool:
        # Coerce to bool, could be 1, 0, True, False, "1", "0", "True", "False"
        return str(os.getenv("PRIMEAGENT_DESKTOP", "False")).lower() in {"1", "true"}

    def _get_client_type(self) -> str:
        return "desktop" if self._get_primeagent_desktop() else "oss"

    async def _send_email_telemetry(self) -> None:
        """Send the telemetry event for the registered email address."""
        from primeagent.utils.registered_email_util import get_email_model
    ...
    def start(self) -> None:
        ...
        try:
            ...
            if self._get_primeagent_desktop():
