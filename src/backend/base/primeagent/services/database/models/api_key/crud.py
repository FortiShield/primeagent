...
async def check_key(session: AsyncSession, api_key: str) -> User | None:
    ...
    Validates API keys based on the PRIMEAGENT_API_KEY_SOURCE setting:
    - 'db': Validates against database-stored API keys (default)
    - 'env': Validates against the PRIMEAGENT_API_KEY environment variable,
...
async def _check_key_from_env(session: AsyncSession, api_key: str, settings_service) -> User | None:
    ...
    PRIMEAGENT_API_KEY environment variable. If valid, returns the superuser for authorization.
    """
    from primeagent.services.database.models.user.crud import get_user_by_username

    env_api_key = os.getenv("PRIMEAGENT_API_KEY")
    if not env_api_key:
        return None
