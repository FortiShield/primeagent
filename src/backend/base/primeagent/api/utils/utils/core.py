...
if TYPE_CHECKING:
    ...
    from primeagent.services.store.schema import StoreComponentCreate
...
async def check_primeagent_version(component: StoreComponentCreate) -> None:
    ...
    primeagent_version = await get_lf_version_from_pypi()
    if primeagent_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Primeagent")
    if primeagent_version != component.last_tested_version:
        await logger.awarning(
            f"Your version of Primeagent ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({primeagent_version}) and try again."
        )
...
async def verify_public_flow_and_get_user(flow_id: uuid.UUID, client_id: str | None) -> tuple[User, uuid.UUID]:
    ...
    async with session_scope() as session:
        from primeagent.services.database.models.flow.model import AccessTypeEnum, Flow
...
def extract_global_variables_from_headers(headers) -> dict[str, str]:
    """Extract global variables from HTTP headers with prefix X-PRIMEAGENT-GLOBAL-VAR-*.
    ...
    try:
        for header_name, header_value in headers.items():
            ...
            if header_lower.startswith(PRIMEAGENT_GLOBAL_VAR_HEADER_PREFIX):
                var_name = header_lower[len(PRIMEAGENT_GLOBAL_VAR_HEADER_PREFIX) :].upper()
