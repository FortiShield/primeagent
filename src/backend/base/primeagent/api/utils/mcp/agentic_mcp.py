"""Utilities for auto-configuring the Primeagent Agentic MCP server."""
...
async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the Primeagent Agentic MCP server for all users.

    This function adds the primeagent-agentic MCP server to each user's MCP
    ...
    await logger.ainfo("Auto-configuring Primeagent Agentic MCP server for all users...")
    ...
    try:
        ...
        server_name = "primeagent-agentic"
        python_executable = sys.executable
        server_config = {
            "command": python_executable,
            "args": ["-m", "primeagent.agentic.mcp"],
            "metadata": {
                "description": "Primeagent Agentic MCP server providing tools for flow/component operations, "
                "template search, and graph visualization",
                "auto_configured": True,
                "primeagent_internal": True,
            },
        }
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the Primeagent Agentic MCP server from all users.

    This function removes the primeagent-agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing Primeagent Agentic MCP server from all users...")
