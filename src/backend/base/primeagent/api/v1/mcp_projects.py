...
from primeagent.api.utils.mcp import (
...
from primeagent.api.v1.mcp import ResponseNoOp
...
async def install_mcp_config(
    ...
    try:
        ...
        if use_mcp_composer:
            ...
            command = "uvx"
            args = ["mcp-proxy"]
            # Check if we need to add Primeagent API key headers
            # Necessary only when Project API Key Authentication is enabled

            # Generate a Primeagent API key for auto-install if needed
            ...
            if should_generate_api_key:
                async with session_scope() as api_key_session:
                    ...
                    primeagent_api_key = api_key_response.api_key
                    args.extend(["--headers", "x-api-key", primeagent_api_key])
        ...
        # Remove stale entries that point to the same Primeagent URLs (e.g. after the project is renamed)
...
class ProjectMCPServer:
    def __init__(self, project_id: UUID):
        ...
        self.server = Server(f"primeagent-mcp-project-{project_id}")
...
async def register_project_with_composer(project: Folder):
    ...
    try:
        ...
        if not settings.host or not settings.port:
            error_msg = "Primeagent host and port must be set in settings to register project with MCP Composer"
            raise ValueError(error_msg)
...
async def get_or_start_mcp_composer(auth_config: dict, project_name: str, project_id: UUID) -> None:
    ...
    """
    from wfx.services.mcp_composer.service import MCPComposerConfigError
