...
from primeagent.services.auth.utils import get_current_user_for_websocket
...
async def authenticate_and_get_openai_key(session: DbSession, user: User, websocket: WebSocket):
    ...
    if user is None:
        await websocket.send_json(
            {
                "type": "error",
                "code": "primeagent_auth",
                "message": "You must pass a valid Primeagent token or cookie.",
            }
        )
        return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "primeagent.voice_mode.config":
                            await logger.ainfo(f"primeagent.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "primeagent.elevenlabs.config":
                            await logger.ainfo(f"primeagent.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "primeagent.elevenlabs.config":
                            await logger.ainfo(f"primeagent.elevenlabs.config {event}")
