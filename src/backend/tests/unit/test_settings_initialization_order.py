...
class TestSettingsInitializationOrder:
    ...
    def test_is_settings_service_initialized_returns_false_initially(self):
        ...
        from primeagent.services.deps import is_settings_service_initialized
    ...
    def test_dotenv_loading_before_settings_init(self, tmp_path):
        ...
        env_file.write_text("PRIMEAGENT_SAVE_DB_IN_CONFIG_DIR=true\n")
        ...
        assert os.environ.get("PRIMEAGENT_SAVE_DB_IN_CONFIG_DIR") == "true"
        ...
        # Clean up
        if "PRIMEAGENT_SAVE_DB_IN_CONFIG_DIR" in os.environ:
            del os.environ["PRIMEAGENT_SAVE_DB_IN_CONFIG_DIR"]
    ...
    def test_cli_check_pattern_success_case(self, tmp_path):
        ...
        env_file.write_text("PRIMEAGENT_DATABASE_URL=sqlite:///./test.db\n")
        ...
        if env_file:
            ...
            if is_settings_service_initialized():
                ...
                load_dotenv(env_file, override=True)
                assert os.environ.get("PRIMEAGENT_DATABASE_URL") == "sqlite:///./test.db"

        # Clean up
        if "PRIMEAGENT_DATABASE_URL" in os.environ:
            del os.environ["PRIMEAGENT_DATABASE_URL"]
...
# Start primeagent run with --env-file in background
...
proc = subprocess.Popen(
    [
        sys.executable, "-m", "primeagent", "run", "--env-file", str(env_file),
