...
import pytest
from primeagent.agentic.helpers.code_extraction import extract_python_code
from primeagent.agentic.helpers.sse import (
    format_complete_event,
    format_error_event,
    format_progress_event,
)
from primeagent.agentic.helpers.validation import validate_component_code
...
# Sample valid Primeagent component code
VALID_COMPONENT_CODE = """from primeagent.custom import Component
...
INVALID_COMPONENT_CODE = """from primeagent.custom import Component
from primeagent.io import MessageTextInput, Output
...
class TestStreamingValidationFlow:
    ...
    async def test_valid_code_first_try_returns_validated(self):
        ...
        with (
            patch(
                "primeagent.agentic.services.assistant_service.classify_intent",
                side_effect=_mock_intent_classification("generate_component"),
            ),
            patch(
                "primeagent.agentic.services.assistant_service.execute_flow_file_streaming",
    ...
    async def test_no_code_in_response_returns_as_is(self):
        ...
        text_only_response = {"result": "Primeagent is a visual flow builder for LLM applications."}
        ...
        with (
            ...
            events = [
                ...
                async for event in execute_flow_with_validation_streaming(
                    flow_filename="test.json",
                    input_value="what is primeagent?",
                    global_variables={},
                    max_retries=3,
                )
            ]
...
class TestNonStreamingValidation:
    ...
    async def test_non_streaming_valid_code_returns_validated(self):
        ...
        with patch(
            "primeagent.agentic.services.assistant_service.execute_flow_file",
