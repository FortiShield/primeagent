...
class TestWorkflowDeveloperAPIProtection:
    ...
    def mock_settings_dev_api_disabled(self):
        ...
        with patch("primeagent.api.v2.workflow.get_settings_service") as mock_get_settings_service:
...
class TestWorkflowBackgroundQueueing:
    ...
    async def test_background_execution_flow(
        ...
        try:
            ...
            with (
                patch("primeagent.api.v2.workflow.get_task_service") as mock_get_task_service,
                patch("primeagent.api.v2.workflow.uuid4", return_value=UUID(mock_job_id)),
            ):
    ...
    async def test_background_execution_queue_exception(
        ...
        try:
            ...
            with patch("primeagent.api.v2.workflow.get_task_service") as mock_get_task_service:
                mock_task_service = MagicMock()
...
class TestWorkflowStatus:
    ...
    async def test_get_status_queued(
        ...
        with patch("primeagent.api.v2.workflow.get_job_service") as mock_get_job_service:
            mock_service = MagicMock()
    ...
    async def test_get_status_completed_reconstruction(
        ...
        with (
            patch("primeagent.api.v2.workflow.get_job_service") as mock_get_job_service,
            patch("primeagent.api.v2.workflow.get_flow_by_id_or_endpoint_name") as mock_get_flow,
            patch("primeagent.api.v2.workflow.reconstruct_workflow_response_from_job_id") as mock_reconstruct,
        ):
            mock_service = MagicMock()
            mock_service.get_job_by_job_id = AsyncMock(return_value=mock_job)
            mock_get_job_service.return_value = mock_service
