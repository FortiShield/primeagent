...
from primeagent.api.v2.files import (
    delete_all_files,
    delete_file,
    delete_files_batch,
...
def max_file_size_upload_fixture(monkeypatch):
    monkeypatch.setenv("PRIMEAGENT_MAX_FILE_SIZE_UPLOAD", "1")
    yield
    monkeypatch.undo()
...
def max_file_size_upload_10mb_fixture(monkeypatch):
    monkeypatch.setenv("PRIMEAGENT_MAX_FILE_SIZE_UPLOAD", "10")
    yield
    monkeypatch.undo()
...
async def s3_files_client_fixture(
    ...
    if "noclient" in request.keywords:
        ...
        def init_app():
            ...
            monkeypatch.setenv("PRIMEAGENT_STORAGE_TYPE", "s3")
            monkeypatch.setenv(
                "PRIMEAGENT_OBJECT_STORAGE_BUCKET_NAME",
                os.environ.get("PRIMEAGENT_OBJECT_STORAGE_BUCKET_NAME", "primeagent-ci"),
            )
            # Use unique prefix per test run to avoid conflicts
            test_prefix = f"test-files-api-{uuid.uuid4().hex[:8]}"
            monkeypatch.setenv("PRIMEAGENT_OBJECT_STORAGE_PREFIX", test_prefix)
            tags_json = json.dumps({"env": "test-api", "type": "file-upload"})
            monkeypatch.setenv("PRIMEAGENT_OBJECT_STORAGE_TAGS", tags_json)

            from primeagent.services.manager import service_manager
        ...
        try:
            ...
            s3 = boto3.client("s3")
            bucket_name = os.environ.get("PRIMEAGENT_OBJECT_STORAGE_BUCKET_NAME", "primeagent-ci")
...
class TestS3FileOperations:
    ...
    async def test_s3_delete_verifies_s3_cleanup(self, s3_files_client, s3_files_created_api_key):
        ...
        test_prefix = os.environ.get("PRIMEAGENT_OBJECT_STORAGE_PREFIX")
