...
@pytest.fixture
def mock_mcp_server():
    with patch("primeagent.api.v1.mcp.server") as mock:
...
async def mock_streamable_http_manager():
    ...
    with patch("primeagent.api.v1.mcp.get_streamable_http_manager", return_value=manager):
        yield manager


@pytest.fixture
def mock_sse_transport():
    with patch("primeagent.api.v1.mcp.sse") as mock:
...
def mock_current_user_ctx(mock_user):
    with patch("primeagent.api.v1.mcp.current_user_ctx") as mock:
...
async def test_streamable_http_start_stop_lifecycle():
    ...
    from primeagent.api.v1.mcp import StreamableHTTP
...
async def test_find_validation_error_with_pydantic_error():
    ...
    import pydantic
    from primeagent.api.v1.mcp import find_validation_error
