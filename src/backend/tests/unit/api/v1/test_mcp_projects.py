...
@pytest.mark.parametrize(
    ("args", "urls", "expected"),
    [
        (None, ["https://primeagent.local/sse"], False),
        ([], ["https://primeagent.local/sse"], False),
        ([123, {"url": "foo"}], ["https://primeagent.local/sse"], False),
        (["https://primeagent.local/sse", 42], ["https://primeagent.local/sse"], True),
        (["alpha", "beta"], [], False),
    ],
)
...
def mock_streamable_http_manager():
    ...
    with patch("primeagent.api.v1.mcp_projects.StreamableHTTPSessionManager") as mock_class:
...
async def test_project_session_manager_lifespan_handles_cleanup(user_test_project, monkeypatch):
    ...
    monkeypatch.setattr(
        "primeagent.api.v1.mcp_projects.StreamableHTTPSessionManager.run",
...
def _prepare_install_test_env(monkeypatch, tmp_path, filename="cursor.json"):
    ...
    monkeypatch.setattr("primeagent.api.v1.mcp_projects.platform.system", lambda: "Linux")
    monkeypatch.setattr("primeagent.api.v1.mcp_projects.should_use_mcp_composer", lambda project: False)  # noqa: ARG005

    async def fake_streamable(project_id):
        return f"https://primeagent.local/api/v1/mcp/project/{project_id}/streamable"
    ...
    monkeypatch.setattr("primeagent.api.v1.mcp_projects.get_settings_service", lambda: dummy_service)
