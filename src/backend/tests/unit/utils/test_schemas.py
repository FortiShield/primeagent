...
class TestChatOutputResponse:
    ...
    def test_validate_files_missing_name_and_type(self):
        ...
        with (
            patch("primeagent.utils.schemas.TEXT_FILE_TYPES", ["pdf", "txt"]),
            patch("primeagent.utils.schemas.IMG_FILE_TYPES", ["jpg", "png"]),
        ):
            response = ChatOutputResponse(message="Test", files=files, type="text")
    ...
    def test_validate_files_unknown_type_raises_error(self):
        ...
        with (
            patch("primeagent.utils.schemas.TEXT_FILE_TYPES", ["txt"]),
            patch("primeagent.utils.schemas.IMG_FILE_TYPES", ["jpg"]),
