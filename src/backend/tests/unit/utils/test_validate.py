...
"""
    ...
    @patch("wfx.custom.validate.logger")
...
"""
        ...
        with patch("wfx.custom.validate.prepare_global_scope") as mock_prepare:
            mock_prepare.return_value = {"CustomComponent": type("CustomComponent", (), {})}
            with patch("wfx.custom.validate.extract_class_code") as mock_extract:
                mock_extract.return_value = Mock()
                with patch("wfx.custom.validate.compile_class_code") as mock_compile:
                    mock_compile.return_value = compile("pass", "<string>", "exec")
                    with patch("wfx.custom.validate.build_class_constructor") as mock_build:
...
"""
        ...
        with (
            patch("wfx.custom.validate.prepare_global_scope", side_effect=validation_error),
...
class TestGetDefaultImports:
    ...
    def test_includes_primeagent_imports(self):
        ...
        with patch("wfx.custom.validate.importlib") as mock_importlib:
            mock_module = Mock()
