...
import pytest
from primeagent.utils.mcp_cleanup import (
    _kill_mcp_processes,
...
class TestCleanupMcpSessions:
    ...
    async def test_cleanup_with_cache_miss(self):
        ...
        with (
            patch("wfx.services.cache.utils.CACHE_MISS", cache_miss_sentinel),
            patch(
                "primeagent.services.deps.get_shared_component_cache_service",
                return_value=mock_cache_service,
            ),
...
class TestKillMcpProcesses:
    ...
    async def test_kills_child_and_orphaned_processes(self):
        ...
        with (
            ...
            patch(
                "primeagent.utils.mcp_cleanup._terminate_child_mcp_processes",
                new_callable=AsyncMock,
                return_value=2,
            ) as mock_child,
            patch(
                "primeagent.utils.mcp_cleanup._terminate_orphaned_mcp_processes",
                new_callable=AsyncMock,
                return_value=1,
            ) as mock_orphan,
        ):
...
class TestTerminateChildMcpProcesses:
    ...
    async def test_terminates_mcp_child_processes(self):
        ...
        with patch(
            "primeagent.utils.mcp_cleanup._try_terminate_mcp_process",
            new_callable=AsyncMock,
