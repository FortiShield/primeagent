...
from wfx.interface.components import aget_all_types_dict, import_primeagent_components
...
class TestComponentLoading:
    ...
    @pytest.mark.asyncio
    async def test_import_primeagent_components_basic(self):
        """Test basic functionality of import_primeagent_components."""
    ...
    async def test_component_loading_performance_comparison(self, base_components_path):
        """Compare performance between import_primeagent_components and aget_all_types_dict."""
        # Warm up the functions (first calls might be slower due to imports)
        await import_primeagent_components()
        await aget_all_types_dict(base_components_path)

        # Time import_primeagent_components
        start_time = time.perf_counter()
        primeagent_result = await import_primeagent_components()
        primeagent_duration = time.perf_counter() - start_time
        ...
        print(f"import_primeagent_components: {primeagent_duration:.4f}s")
        print(f"aget_all_types_dict: {all_types_duration:.4f}s")
        print(f"Ratio (primeagent/all_types): {primeagent_duration / max(all_types_duration, 0.0001):.2f}")
        ...
        return {
            "primeagent_result": primeagent_result,
            "all_types_result": all_types_result,
            "primeagent_duration": primeagent_duration,
    ...
    async def test_result_structure_comparison(self, base_components_path):
        ...
        # Check primeagent result structure
        assert isinstance(primeagent_result, dict)
        assert "components" in primeagent_result
        primeagent_components = primeagent_result["components"]

        # Check all_types result structure
        assert isinstance(all_types_result, dict)

        # Get component counts (informational, non-failing)
        primeagent_count = sum(len(comps) for comps in primeagent_components.values())
        ...
        print(f"import_primeagent_components: {primeagent_count} components")
        print(f"aget_all_types_dict: {all_types_count} components")

        # Log the comparison but don't fail the test
        if primeagent_count != all_types_count:
            diff = abs(primeagent_count - all_types_count)
            print(f"Note: Component counts differ by {diff} - this may be due to OS file limits")

        # Analyze component categories
        if primeagent_components:
            primeagent_categories = list(primeagent_components.keys())
            print(f"Primeagent categories: {sorted(primeagent_categories)}")
        ...
        for category, components in primeagent_components.items():
    ...
    async def test_component_template_structure(self):
        ...
        for category, components in primeagent_result["components"].items():
            assert isinstance(components, dict), f"Category {category} should contain dict of components"
    ...
    async def test_concurrent_loading(self, base_components_path):
        ...
        tasks = [
            ...
            import_primeagent_components(),  # Run primeagent loader twice to test consistency
        ]
        ...
        primeagent_result1, all_types_result, primeagent_result2 = results

        print(f"\nConcurrent execution took: {concurrent_duration:.4f}s")

        # Check that both results have the same structure and component counts
        assert isinstance(primeagent_result1, dict)
        assert isinstance(primeagent_result2, dict)
        assert isinstance(all_types_result, dict)

        # Check that both primeagent results have the same component structure
        assert "components" in primeagent_result1
        assert "components" in primeagent_result2

        # Compare component counts (informational, non-failing)
        count1 = sum(len(comps) for comps in primeagent_result1["components"].values())
        count2 = sum(len(comps) for comps in primeagent_result2["components"].values())
        ...
        categories1 = set(primeagent_result1["components"].keys())
        categories2 = set(primeagent_result2["components"].keys())
        ...
        for category in categories1.intersection(categories2):
            comps1 = set(primeagent_result1["components"][category].keys())
            comps2 = set(primeagent_result2["components"][category].keys())
            if comps1 != comps2:
    ...
    async def test_memory_efficiency(self, base_components_path):
        ...
        # Load with import_primeagent_components
        primeagent_result = await import_primeagent_components()
        after_primeagent_objects = len(gc.get_objects())
        ...
        primeagent_objects_created = after_primeagent_objects - initial_objects
        all_types_objects_created = after_all_types_objects - after_primeagent_objects

        print("\nMemory Analysis:")
        print(f"Objects created by import_primeagent_components: {primeagent_objects_created}")
        print(f"Objects created by aget_all_types_dict: {all_types_objects_created}")

        # Clean up
        del primeagent_result, all_types_result
        gc.collect()
    ...
    async def test_error_handling(self):
        ...
        # import_primeagent_components should work regardless of external paths
        result = await import_primeagent_components()
    ...
    async def test_repeated_loading_performance(self, base_components_path):
        ...
        # Test repeated import_primeagent_components calls
        primeagent_times = []
        for _ in range(num_iterations):
            start_time = time.perf_counter()
            await import_primeagent_components()
            duration = time.perf_counter() - start_time
            primeagent_times.append(duration)
        ...
        primeagent_avg = sum(primeagent_times) / len(primeagent_times)
        primeagent_min = min(primeagent_times)
        primeagent_max = max(primeagent_times)
        ...
        print(
            f"import_primeagent_components - avg: {primeagent_avg:.4f}s, min: {primeagent_min:.4f}s, max: {primeagent_max:.4f}s"
        )
        print(f"aget_all_types_dict - avg: {all_types_avg:.4f}s, min: {all_types_min:.4f}s, max: {all_types_max:.4f}s")

        # Performance should be reasonably consistent
        primeagent_variance = max(primeagent_times) - min(primeagent_times)
    ...
    async def test_comprehensive_performance_summary(self, base_components_path):
        ...
        await import_primeagent_components()  # Warm up imports, thread pools, etc.
        ...
        for run in range(num_runs):
            ...
            primeagent_result = await import_primeagent_components()
            primeagent_duration = time.perf_counter() - start_time
            primeagent_results.append((primeagent_duration, primeagent_result))
        ...
        primeagent_times = [duration for duration, _ in primeagent_results]
        all_types_times = [duration for duration, _ in all_types_results]

        print("\nSTEADY-STATE PERFORMANCE (after warm-up):")
        print("import_primeagent_components:")
        print(f"  Average: {sum(primeagent_times) / len(primeagent_times):.4f}s")
        print(f"  Min: {min(primeagent_times):.4f}s")
        print(f"  Max: {max(primeagent_times):.4f}s")
        ...
        primeagent_component_counts = []
        all_types_component_counts = []

        for _, result in primeagent_results:
            count = sum(len(comps) for comps in result.get("components", {}).values())
            primeagent_component_counts.append(count)
        ...
        print("\nCOMPONENT COUNTS:")
        print(f"import_primeagent_components: {primeagent_component_counts}")
        print(f"aget_all_types_dict: {all_types_component_counts}")

        # Determine which is faster (based on steady-state performance)
        avg_primeagent = sum(primeagent_times) / len(primeagent_times)
        avg_all_types = sum(all_types_times) / len(all_types_times)

        if avg_primeagent < avg_all_types:
            faster_method = "import_primeagent_components"
            speedup = avg_all_types / avg_primeagent
        else:
            faster_method = "aget_all_types_dict"
            speedup = avg_primeagent / avg_all_types
        ...
        if primeagent_component_counts:
            min_count = min(primeagent_component_counts)
            max_count = max(primeagent_component_counts)
        ...
        print(f"Steady-state performance: avg_primeagent={avg_primeagent:.4f}s, speedup={speedup:.2f}x")
    ...
    async def test_component_differences_analysis(self, base_components_path):
        ...
        # import_primeagent_components returns {"components": {category: {comp_name: comp_data}}}
        # aget_all_types_dict returns {category: {comp_name: comp_data}}
        primeagent_components = primeagent_result.get("components", {})
        ...
        for category, components in primeagent_components.items():
            for comp_name in components:
                primeagent_flat[comp_name] = category
        ...
        # Calculate counts
        primeagent_count = len(primeagent_flat)
        all_types_count = len(all_types_flat)

        print("\nCOMPONENT COUNTS:")
        print(f"import_primeagent_components: {primeagent_count} components")
        print(f"aget_all_types_dict: {all_types_count} components")
        print(f"Difference: {abs(primeagent_count - all_types_count)} components")

        # Find components that are in one but not the other
        primeagent_only = set(primeagent_flat.keys()) - set(all_types_flat.keys())
        all_types_only = set(all_types_flat.keys()) - set(primeagent_flat.keys())
        common_components = set(primeagent_flat.keys()) & set(all_types_flat.keys())

        print("\nCOMPONENT OVERLAP:")
        print(f"Common components: {len(common_components)}")
        print(f"Only in import_primeagent_components: {len(primeagent_only)}")
        print(f"Only in aget_all_types_dict: {len(all_types_only)}")

        # Print detailed differences
        if primeagent_only:
            print(f"\nCOMPONENTS ONLY IN import_primeagent_components ({len(primeagent_only)}):")
            for comp_name in sorted(primeagent_only):
                category = primeagent_flat[comp_name]
        ...
        for comp_name in common_components:
            primeagent_cat = primeagent_flat[comp_name]
            all_types_cat = all_types_flat[comp_name]
            if primeagent_cat != all_types_cat:
                category_differences.append((comp_name, primeagent_cat, all_types_cat))

        if category_differences:
            print(f"\nCOMPONENTS WITH DIFFERENT CATEGORIES ({len(category_differences)}):")
            for comp_name, primeagent_cat, all_types_cat in sorted(category_differences):
                print(f"  - {comp_name}: import_primeagent='{primeagent_cat}' vs aget_all_types='{all_types_cat}'")

        # Print category summary
        print("\nCATEGORY SUMMARY:")
        primeagent_categories = set(primeagent_components.keys())
        all_types_categories = set(all_types_components.keys())

        print(f"Categories in import_primeagent_components: {sorted(primeagent_categories)}")
        print(f"Categories in aget_all_types_dict: {sorted(all_types_categories)}")

        categories_only_primeagent = primeagent_categories - all_types_categories
        categories_only_all_types = all_types_categories - primeagent_categories

        if categories_only_primeagent:
            print(f"Categories only in import_primeagent_components: {sorted(categories_only_primeagent)}")
        ...
        if primeagent_count == 0 and all_types_count == 0:
            print("Note: Both methods returned 0 components - this may be due to OS file limits")
        elif len(common_components) == 0 and (primeagent_count > 0 or all_types_count > 0):
    ...
    async def test_process_single_module_exception_handling(self):
        ...
        missing dependencies, or initialization failures), it doesn't crash Primeagent startup.
        """
        from unittest.mock import patch

        from wfx.interface.components import _process_single_module
        ...
        print("Component failures will not crash Primeagent startup")
        print("=" * 80)
