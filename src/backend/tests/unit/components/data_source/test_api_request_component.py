...
from wfx.components.data_source.api_request import APIRequestComponent
...
class TestAPIRequestSSRFProtection:
    ...
    async def test_ssrf_protection_enabled_blocks_localhost(self, component):
        ...
        with (
            ...
            patch("wfx.components.data_source.api_request.validate_url_for_ssrf") as mock_validate,
        ):
    ...
    async def test_ssrf_protection_enabled_blocks_private_networks(self, component):
        ...
        with patch.dict(os.environ, {"PRIMEAGENT_SSRF_PROTECTION_ENABLED": "true"}):
            for url in private_ips:
                component.url_input = url

                with patch("wfx.components.data_source.api_request.validate_url_for_ssrf") as mock_validate:
                    from wfx.utils.ssrf_protection import SSRFProtectionError
    ...
    async def test_ssrf_protection_allows_public_urls(self, component):
        ...
        public_urls = [
            ...
            "https://api.github.com/repos/khulnasoft-bot/primeagent",
