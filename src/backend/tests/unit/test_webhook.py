...
import anyio
import pytest
from primeagent.services.event_manager import WebhookEventManager
...
class TestGetVertexIdsFromFlow:
    ...
    def test_returns_empty_list_when_flow_data_is_none(self):
        ...
        from primeagent.api.v1.endpoints import _get_vertex_ids_from_flow
...
class TestSimpleRunFlowTask:
    ...
    async def test_emits_vertices_sorted_event_when_emit_events_true(self):
        ...
        from primeagent.api.v1.endpoints import simple_run_flow_task
        ...
        with (
            patch("primeagent.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run,
    ...
    async def test_emits_error_event_on_exception(self):
        ...
        with (
            patch("primeagent.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run,
            patch("primeagent.api.v1.endpoints.webhook_event_manager") as mock_manager,
            patch("primeagent.api.v1.endpoints.logger") as mock_logger,
        ):
    ...
    async def test_logs_telemetry_on_success(self):
        ...
        with patch("primeagent.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run:
            mock_run.return_value = {"result": "success"}
...
class TestWebhookEventsStreamAuth:
    ...
    async def test_calls_get_current_user_for_sse_for_authentication(self):
        ...
        from primeagent.api.v1.endpoints import webhook_events_stream
