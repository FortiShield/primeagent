...
import pytest
import typer
from primeagent.__main__ import _create_superuser, app, get_number_of_workers
from wfx.services import deps
...
def test_get_number_of_workers():
    ...
    with (
        patch("primeagent.__main__.platform.system", return_value="Linux"),
        patch("primeagent.__main__.cpu_count", return_value=4),
    ):
