...
- API_KEY_SOURCE='env': Validates against PRIMEAGENT_API_KEY environment variable
"""
...
def mock_settings_service_db():
    ...
    settings_service.auth_settings.SUPERUSER = "primeagent"
...
class TestCheckKeyRouting:
    ...
    async def test_check_key_routes_to_db_by_default(self, mock_session, mock_settings_service_db):
        ...
        with (
            patch(
                "primeagent.services.database.models.api_key.crud.get_settings_service",
                return_value=mock_settings_service_db,
            ),
            patch(
                "primeagent.services.database.models.api_key.crud._check_key_from_db",
                new_callable=AsyncMock,
            ) as mock_db_check,
            patch(
                "primeagent.services.database.models.api_key.crud._check_key_from_env",
                new_callable=AsyncMock,
            ) as mock_env_check,
        ):
...
class TestCheckKeyFromEnv:
    ...
    async def test_valid_key_returns_superuser(
        ...
        monkeypatch.setenv("PRIMEAGENT_API_KEY", "sk-test-env-key")

        with patch(
            "primeagent.services.database.models.user.crud.get_user_by_username",
            new_callable=AsyncMock,
        ) as mock_get_user:
            ...
            mock_get_user.assert_called_once_with(mock_session, "primeagent")
    ...
    async def test_no_env_api_key_configured_returns_none(self, mock_session, mock_settings_service_env, monkeypatch):
        """When PRIMEAGENT_API_KEY is not set, should return None."""
    ...
    async def test_empty_env_api_key_returns_none(self, mock_session, mock_settings_service_env, monkeypatch):
        """When PRIMEAGENT_API_KEY is empty string, should return None."""
        monkeypatch.setenv("PRIMEAGENT_API_KEY", "")
    ...
    async def test_case_sensitive_key_comparison(self, mock_session, mock_settings_service_env, monkeypatch):
        ...
        monkeypatch.setenv("PRIMEAGENT_API_KEY", "sk-Test-Key")
    ...
    async def test_special_characters_in_key(
        ...
        monkeypatch.setenv("PRIMEAGENT_API_KEY", special_key)
    ...
    async def test_unicode_in_key(self, mock_session, mock_superuser, mock_settings_service_env, monkeypatch):
        ...
        monkeypatch.setenv("PRIMEAGENT_API_KEY", unicode_key)
    ...
    async def test_very_long_key(self, mock_session, mock_superuser, mock_settings_service_env, monkeypatch):
        ...
        long_key = "sk-" + "a" * 1000
        monkeypatch.setenv("PRIMEAGENT_API_KEY", long_key)
...
class TestCheckKeyIntegration:
    ...
    async def test_full_flow_env_mode_valid_key(self, mock_session, mock_superuser, monkeypatch):
        ...
        monkeypatch.setenv("PRIMEAGENT_API_KEY", "sk-env-secret")

        mock_settings = MagicMock()
        mock_settings.auth_settings.API_KEY_SOURCE = "env"
        mock_settings.auth_settings.SUPERUSER = "primeagent"
    ...
    async def test_full_flow_env_mode_invalid_key_falls_back_to_db(self, mock_session, mock_user, monkeypatch):
        ...
        monkeypatch.setenv("PRIMEAGENT_API_KEY", "sk-correct-key")
        ...
        monkeypatch.setattr(
            "primeagent.services.database.models.api_key.crud.auth_utils.decrypt_api_key",
