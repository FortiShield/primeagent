#!/usr/bin/env python3
"""Example Primeagent Load Testing Workflow

This script demonstrates the complete workflow for setting up and running
Primeagent load tests with real starter project flows.
...
def main():
    ...
    setup_script = script_dir / "primeagent_setup_test.py"
    runner_script = script_dir / "primeagent_run_load_test.py"
    ...
    try:
        ...
        try:
            ...
            env = os.environ.copy()
            env["PRIMEAGENT_HOST"] = creds["host"]
            env["API_KEY"] = creds["api_key"]
            env["FLOW_ID"] = creds["flow_id"]

            print("   🔧 Setting environment variables:")
            print(f"      PRIMEAGENT_HOST={creds['host']}")
        ...
        success = run_command_with_env(
            [
                ...
                "--no-start-primeagent",
                "--html",
                "primeagent_load_test_report.html",
                "--csv",
                "primeagent_load_test_results",
            ],
        ...
        print("   python setup_primeagent_test.py --interactive")
        print()
        ...
        print("   - primeagent_load_test_report.html (detailed HTML report)")
        print("   - primeagent_load_test_results_*.csv (CSV data files)")
        print("   - primeagent_load_test_detailed_errors_*.log (detailed error logs)")
        print("   - primeagent_load_test_error_summary_*.json (error analysis)")
        print("   - primeagent_server_logs_during_test_*.log (Primeagent server logs)")
        print()
        print("🧹 Clean up:")
        print("   - Remove test flows from Primeagent UI")
        ...
        if args.auto:
            ...
            print("   - primeagent_load_test_report.html")
            print("   - primeagent_load_test_results_*.csv")
        else:
            ...
            if cleanup_response == "y":
                files_to_clean = [
                    "example_test_creds.json",
                    "primeagent_load_test_report.html",
                    "primeagent_load_test_results_failures.csv",
                    "primeagent_load_test_results_stats.csv",
                    "primeagent_load_test_results_stats_history.csv",
                    "primeagent_load_test_results_exceptions.csv",
                ]

                # Also clean up error logs (they have timestamps, so use glob pattern)
                import glob

                error_files = glob.glob("primeagent_load_test_detailed_errors_*.log")
                error_files.extend(glob.glob("primeagent_load_test_error_summary_*.json"))
                error_files.extend(glob.glob("primeagent_server_logs_during_test_*.log"))
        ...
        print("You're now ready to use Primeagent load testing for your own projects.")
        print()
        print("📊 View your load test results:")
        print("   • Open primeagent_load_test_report.html in your browser for detailed analysis")
        print("   • Check primeagent_load_test_results_*.csv for raw data")
        print("   • Review primeagent_load_test_detailed_errors_*.log for comprehensive error details")
        print("   • Analyze primeagent_load_test_error_summary_*.json for error patterns")
        print("   • Examine primeagent_server_logs_during_test_*.log for server-side issues")
