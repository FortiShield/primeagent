...
from wfx.base.agents.utils import data_to_messages, get_chat_output_sender_name
from wfx.components.models_and_agents import AgentComponent
...
if TYPE_CHECKING:
    ...
    from wfx.schema.log import SendMessageFunctionType
...
def normalize_message_content(message: BaseMessage) -> str:
    ...
    Note:
        This addresses the inconsistency in wfx.schema.data.Data.to_lc_message() where:
...
class ALTKBaseTool(BaseTool):
    ...
    def _get_altk_llm_object(self, *, use_output_val: bool = True) -> Any:
        ...
        if isinstance(llm_object, ChatAnthropic):
            ...
            logger.info("ALTK currently only supports OpenAI and Anthropic models through Primeagent.")
            llm_client_obj = None
