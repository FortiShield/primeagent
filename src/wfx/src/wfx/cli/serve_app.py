"""FastAPI application factory for serving **multiple** WFX graphs at once.
...
def _analyze_graph_structure(graph: Graph) -> dict[str, Any]:
    ...
    Args:
        graph: The WFX graph to analyze
...
def _generate_dynamic_run_description(graph: Graph) -> str:
    ...
    description_parts = [
        f"Execute the deployed WFX graph with {analysis['node_count']} components.",
        "",
        "**Graph Analysis**:",
...
class RunRequest(BaseModel):
    """Request model for executing a WFX flow."""

    input_value: str = Field(..., description="Input value passed to the flow")


class StreamRequest(BaseModel):
    """Request model for streaming execution of a WFX flow."""
...
async def run_flow_generator_for_serve(
    ...
    try:
        ...
        # to integrate with the full WFX streaming pipeline from endpoints.py
...
def create_multi_serve_app(
    *,
    ...
    """Create a FastAPI app exposing multiple WFX flows.
    ...
    app = FastAPI(
        title=f"WFX Multi-Flow Server ({len(graphs)})",
        description=(
            "This server hosts multiple WFX graphs under the `/flows/{id}` prefix. "
    ...
    def create_flow_router(flow_id: str, graph: Graph, meta: FlowMeta) -> APIRouter:
        ...
        async def stream_flow(
            ...
            try:
                ...
                from wfx.events.event_manager import create_stream_tokens_event_manager
