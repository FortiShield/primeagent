...
from wfx.cli.serve_app import FlowMeta, create_multi_serve_app
...
def serve_command(
    ...
    *,
    ...
    Examples:
        ...
        # Serve inline JSON
        wfx serve --flow-json '{"nodes": [...], "edges": [...]}'

        # Serve from stdin
        cat my_flow.json | wfx serve --stdin
        echo '{"nodes": [...]}' | wfx serve --stdin
    """
    ...
    try:
        ...
        if resolved_path.suffix == ".json":
            ...
            from wfx.cli.script_loader import load_graph_from_script
        ...
        try:
            ...
            if check_variables:
                from wfx.cli.validation import validate_global_variables_for_env
        ...
        console.print(
            Panel.fit(
                ...
                title="[bold blue]WFX Server[/bold blue]",
                border_style="blue",
            )
        )
        console.print()
