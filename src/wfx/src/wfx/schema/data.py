...
class Data(CrossModuleModel):
    ...
    def to_lc_message(
        ...
        if sender == MESSAGE_SENDER_USER:
            if files:
                from wfx.schema.image import get_file_paths
    ...
    def filter_data(self, filter_str: str) -> Data:
        ...
        """
        from wfx.template.utils import apply_json_filter

        return apply_json_filter(self.data, filter_str)

    def to_message(self) -> Message:
        from wfx.schema.message import Message  # Local import to avoid circular import
