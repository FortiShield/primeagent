...
from wfx.graph.schema import INPUT_COMPONENTS, OUTPUT_COMPONENTS, InterfaceComponentTypes, ResultData
from wfx.graph.utils import UnbuiltObject, UnbuiltResult, emit_build_start_event, log_transaction
from wfx.graph.vertex.param_handler import ParameterHandler
from wfx.interface import initialize
from wfx.interface.listing import lazy_load_dict
from wfx.log.logger import logger
from wfx.schema.artifact import ArtifactType
from wfx.schema.data import Data
from wfx.schema.message import Message
from wfx.schema.schema import INPUT_FIELD_NAME, OutputValue, build_output_logs
from wfx.utils.schemas import ChatOutputResponse
from wfx.utils.util import sync_to_async
...
class Vertex:
    ...
    async def build(
        ...
        **kwargs,
        ...
        from wfx.interface.components import ensure_component_loaded
    ...
    def before_callback_event(self, *args, **kwargs) -> StepStartedEvent:  # noqa: ARG002
        ...
        return StepStartedEvent(step_name=self.display_name, raw_event={"primeagent": metrics})
    ...
    def after_callback_event(self, result, *args, **kwargs) -> StepFinishedEvent:  # noqa: ARG002
        ...
        if hasattr(self, "raw_event_metrics"):
            metrics = self.raw_event_metrics({"component_id": self.id})
        return StepFinishedEvent(step_name=self.display_name, raw_event={"primeagent": metrics})
