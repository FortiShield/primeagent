...
from wfx.custom.dependency_analyzer import analyze_component_dependencies
from wfx.custom.directory_reader.utils import (
...
from wfx.custom.schema import MissingDefault
...
from wfx.template.frontend_node.custom_components import ComponentFrontendNode, CustomComponentFrontendNode
from wfx.type_extraction.type_extraction import extract_inner_type
from wfx.utils.util import get_base_classes
...
def run_build_config(
    ...
    # we have imported it
    # If not, it means the component was loaded through PRIMEAGENT_COMPONENTS_PATH
...
async def get_single_component_dict(component_type: str, component_name: str, components_paths: list[str]):
    ...
    for base_path in components_paths:
        ...
        if module_path.exists():
            ...
            module_name = f"wfx.components.{component_type}.{component_name}"
            try:
...
async def load_custom_component(component_name: str, components_paths: list[str]):
    ...
    """
    from wfx.interface.custom_component import get_custom_component_from_name
                ...
                for category_dir in base_path.iterdir():
                    if category_dir.is_dir():
                        ...
                        if component_file.exists():
                            # Try to import the module
                            module_name = f"wfx.components.{category_dir.name}.{component_name}"
                            try:
                                import importlib.util
