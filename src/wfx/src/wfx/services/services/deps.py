"""Service dependency functions for wfx package."""
...
if TYPE_CHECKING:
    ...
    from wfx.services.interfaces import (
        AuthServiceProtocol,
...
def get_db_service() -> DatabaseServiceProtocol:
    ...
    if db_service is None:
        ...
        # This allows wfx to work in standalone mode without requiring database setup
...
def get_shared_component_cache_service() -> CacheServiceProtocol | None:
    ...
    from wfx.services.shared_component_cache.factory import SharedComponentCacheServiceFactory
...
def get_auth_service() -> AuthServiceProtocol | None:
    ...
    Returns the pluggable auth service (minimal WFX or full Primeagent when configured).
    """
    from wfx.services.schema import ServiceType
