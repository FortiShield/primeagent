...
2. Config file (wfx.toml / pyproject.toml)
...
class ServiceManager:
    ...
    def discover_plugins(self, config_dir: Path | None = None) -> None:
        ...
        2. Config files (wfx.toml / pyproject.toml)
    ...
    def _discover_from_entry_points(self) -> None:
        ...
        eps = entry_points(group="wfx.services")
    ...
    def _discover_from_config(self, config_dir: Path | None = None) -> None:
        """Discover services from config files (wfx.toml / pyproject.toml)."""
        config_dir = Path.cwd() if config_dir is None else Path(config_dir)

        # Try wfx.toml first
        wfx_config = config_dir / "wfx.toml"
        if wfx_config.exists():
            self._load_config_file(wfx_config)
            return

        # Try pyproject.toml with [tool.wfx.services]
    ...
    def _load_config_file(self, config_path: Path) -> None:
        """Load services from wfx.toml config file."""
        try:
    ...
    def _load_pyproject_config(self, config_path: Path) -> None:
        """Load services from pyproject.toml [tool.wfx.services] section."""
        try:
            import tomllib as tomli  # Python 3.11+
        except ImportError:
            import tomli  # Python 3.10
        ...
        try:
            ...
            services = config.get("tool", {}).get("wfx", {}).get("services", {})
            for service_key, service_path in services.items():
                self._register_service_from_path(service_key, service_path)
    ...
    def _register_service_from_path(self, service_key: str, service_path: str) -> None:
        ...
        Args:
            ...
            service_path: Import path (e.g., "primeagent.services.database.service:DatabaseService")
        """
        try:
