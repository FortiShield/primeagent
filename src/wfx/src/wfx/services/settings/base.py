...
from wfx.services.settings.constants import AGENTIC_VARIABLES, VARIABLES_TO_GET_FROM_ENVIRONMENT
from wfx.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    ...
    If not provided, a hash of the database URL will be used. Useful when multiple Primeagent
    ...
    If None, uses the built-in index at wfx/_assets/component_index.json.
    ...
    object_storage_bucket_name: str | None = "primeagent-bucket"
    """Object storage bucket name for file storage. Defaults to 'primeagent-bucket'."""
    ...
    """If set to True, Primeagent will start the agentic MCP server that provides tools for
    flow/component operations, template search, and graph visualization."""

    # Developer API
    developer_api_enabled: bool = False
    """If set to True, Primeagent will enable developer API endpoints for advanced debugging and introspection."""
    ...
    """If set to True, Primeagent will enable SSRF (Server-Side Request Forgery) protection.
    ...
    def set_variables_to_get_from_environment(cls, value):
        ...
        if os.getenv("PRIMEAGENT_AGENTIC_EXPERIENCE", "true").lower() == "true":
