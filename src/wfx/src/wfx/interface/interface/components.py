...
from wfx.custom.utils import abuild_custom_components, create_component_template
from wfx.log.logger import logger
from wfx.utils.validate_cloud import (
...
MIN_MODULE_PARTS = 2
MIN_MODULE_PARTS_WITH_FILENAME = 4  # Minimum parts needed to have a module filename (wfx.components.type.filename)
...
def _parse_dev_mode() -> tuple[bool, list[str] | None]:
    """Parse WFX_DEV to determine dev mode and which modules to load.

    Development mode must be explicitly enabled via the WFX_DEV environment variable.
    ...
    Supports two modes:
    - Boolean mode: WFX_DEV=1/true/yes loads all modules dynamically
    - List mode: WFX_DEV=mistral,openai,anthropic loads only specified modules
...
def _read_component_index(custom_path: str | None = None) -> dict | None:
    ...
    try:
        ...
        if custom_path:
            ...
            # Use built-in index
            pkg_dir = Path(inspect.getfile(wfx)).parent
...
async def _load_full_dev_mode() -> tuple[dict[str, Any], str]:
    ...
    """
    await logger.adebug("WFX_DEV full mode: loading all modules dynamically")
...
async def _load_selective_dev_mode(
    ...
    Returns:
        Tuple of (modules_dict, index_source)
    """
    await logger.adebug(f"WFX_DEV selective mode: reloading {target_modules}")
...
async def import_primeagent_components(
    ...
    """Asynchronously discovers and loads all built-in Primeagent components.
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from wfx.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from wfx.custom.utils import build_custom_components
