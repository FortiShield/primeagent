...
WFX now supports a pluggable service architecture that allows you to customize and extend service implementations without modifying core code.
...
Create an `wfx.toml` in your project root:

```toml
[services]
database_service = "primeagent.services.database.service:DatabaseService"
storage_service = "primeagent.services.storage.local:LocalStorageService"
cache_service = "primeagent.services.cache.service:ThreadingInMemoryCache"
```

Then run:
```bash
wfx serve my_flow.json
```

Services will be automatically discovered from:
1. The current working directory (looking for `wfx.toml` or `pyproject.toml`)
...
```python
from wfx.services import register_service
...
```toml
[project.entry-points."wfx.services"]
database_service = "my_plugin.services:MyDatabaseService"
storage_service = "my_plugin.services:MyStorageService"
```

When your package is installed, WFX will automatically discover these services.

## Configuration Details

### Config File Locations

WFX searches for configuration in this order:

1. `./wfx.toml` (project-specific config)
2. `./pyproject.toml` under `[tool.wfx.services]` (Python project integration)
...
3. Ensure config file is in the correct location:
   - If settings service exists: `{settings.config_dir}/wfx.toml`
   - Otherwise: `./wfx.toml` or `./pyproject.toml` in current directory
4. Enable debug logging: `export PRIMEAGENT_LOG_LEVEL=DEBUG`

### Import Errors

**Error:** `ModuleNotFoundError: No module named 'primeagent'`

**Solutions:**
1. Ensure primeagent is installed: `pip install primeagent`
2. Check if module path in config is correct
3. Verify the package is importable: `python -c "import primeagent.services.database"`
...
See:
- `wfx.toml.example` - Example configuration file showing Primeagent service registration
- `src/wfx/services/` - Minimal built-in service implementations (auth, telemetry, tracing, variable, storage, etc.)
  - Auth: `wfx.services.auth.base` (BaseAuthService) and `wfx.services.auth.service` (AuthService). Use `get_auth_service()` from `wfx.services.deps`. Override with `auth_service = "primeagent.services.auth.service:AuthService"` in config for full JWT/API key auth.
- `src/backend/base/primeagent/services/` - Full-featured Primeagent services
