...
def pytest_configure(config):
    ...
    if not os.getenv("WFX_TEST_ALLOW_PRIMEAGENT"):
        try:
...
def use_noop_database():
    """Ensure all WFX tests use NoopDatabaseService.
    ...
    with patch("wfx.services.deps.get_db_service", return_value=NoopDatabaseService()):
        yield
...
def use_noop_session():
    ...
    with patch("wfx.services.deps.session_scope") as mock_session_scope:
