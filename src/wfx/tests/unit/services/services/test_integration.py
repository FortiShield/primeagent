...
from wfx.services.manager import ServiceManager
from wfx.services.schema import ServiceType

from .conftest import MockSessionService


class TestStandaloneWFX:
    """Test WFX running standalone without primeagent."""
    ...
    def test_minimal_storage_service_loads(self, clean_manager):
        ...
        from wfx.services.storage.local import LocalStorageService
    ...
    def test_minimal_telemetry_service_loads(self, clean_manager):
        ...
        from wfx.services.telemetry.service import TelemetryService
    ...
    def test_minimal_variable_service_loads(self, clean_manager):
        ...
        from wfx.services.variable.service import VariableService
...
class TestWFXWithPrimeagentConfig:
    """Test WFX with primeagent configuration."""

    @pytest.fixture
    def primeagent_config_dir(self, tmp_path):
        """Create a temporary primeagent-style config directory."""
...
""")
    ...
    def test_config_overrides_defaults(self, clean_manager, primeagent_config_dir):
        """Test that config file overrides default services."""
        # Discover plugins from config
        clean_manager.discover_plugins(primeagent_config_dir)
    ...
    def test_multiple_services_from_config(self, clean_manager, primeagent_config_dir):
...
""")
    ...
    def test_wfx_toml_takes_precedence(self, clean_manager, tmp_path):
        """Test that wfx.toml takes precedence over pyproject.toml."""
        config_dir = tmp_path / "config"
        config_dir.mkdir()
...
""")
        ...
        # Should only have storage from wfx.toml
        assert ServiceType.STORAGE_SERVICE in clean_manager.service_classes
