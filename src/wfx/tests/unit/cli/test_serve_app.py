"""Unit tests for WFX CLI FastAPI serve app."""
...
class TestSecurityFunctions:
    ...
    def test_verify_api_key_invalid(self):
        ...
        with patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "correct-key"}):  # pragma: allowlist secret
            with pytest.raises(HTTPException) as exc_info:
    ...
    def test_verify_api_key_env_not_set(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert "PRIMEAGENT_API_KEY environment variable is not set" in exc_info.value.detail
...
class TestCreateServeApp:
    ...
    def test_create_multi_serve_app_single_flow(self, real_graph, mock_meta):
        ...
        assert app.title == "WFX Multi-Flow Server (1)"
    ...
    def test_create_multi_serve_app_multiple_flows(self, real_graph, mock_meta, simple_chat_json):
        ...
        assert app.title == "WFX Multi-Flow Server (2)"
        assert "Use `/flows` to list available IDs" in app.description
...
class TestServeAppEndpoints:
    ...
    def app_client(self, real_graph_with_async):
        ...
        # Set up test API key
        with patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-api-key"}):  # pragma: allowlist secret
    ...
    def test_run_endpoint_success(self, app_client):
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-api-key"}),  # pragma: allowlist secret
            patch("wfx.cli.common.extract_structured_result") as mock_extract,
        ):
    ...
    def test_run_endpoint_execution_error(self, app_client):
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-api-key"}),  # pragma: allowlist secret
            patch("wfx.cli.serve_app.execute_graph_with_capture", mock_execute_error),
        ):
            response = app_client.post("/flows/test-flow-id/run", json=request_data, headers=headers)
    ...
    def test_run_endpoint_no_results(self, app_client):
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-api-key"}),  # pragma: allowlist secret
            patch("wfx.cli.serve_app.execute_graph_with_capture", mock_execute_empty),
        ):
            response = app_client.post("/flows/test-flow-id/run", json=request_data, headers=headers)
