"""Integration tests for wfx CLI with Simple Agent flow.

These tests verify that the wfx CLI can properly load and execute the
Simple Agent starter project, addressing the bug where wfx serve/run
...
- wfx requires langchain-core>=0.3.66,<1.0.0
...
class TestSimpleAgentFlowLoading:
    ...
    def test_simple_agent_flow_loads_via_cli(self, simple_agent_flow_path: Path):
        """Test that wfx run can load the Simple Agent flow without critical errors."""
        ...
        critical_errors = [
            "No module named 'wfx.components",
            "No module named 'primeagent",
    ...
    def test_simple_agent_flow_loads_directly(self, simple_agent_flow_path: Path):
        ...
        from wfx.load import load_flow_from_json
    ...
    def test_simple_agent_flow_json_output(self, simple_agent_flow_path: Path):
        """Test that wfx run produces valid JSON output."""
        result = runner.invoke(
            wfx_app,
...
class TestSimpleAgentServe:
    ...
    def test_serve_help(self):
        ...
        result = runner.invoke(wfx_app, ["serve", "--help"])
    ...
    def test_serve_requires_api_key(self, simple_agent_flow_path: Path, monkeypatch):
        """Test serve requires PRIMEAGENT_API_KEY."""
        monkeypatch.delenv("PRIMEAGENT_API_KEY", raising=False)
        ...
        assert result.exit_code != 0 or "PRIMEAGENT_API_KEY" in result.output
    ...
    def test_serve_starts_server_no_asyncio_error(self, simple_agent_flow_path: Path):
        """Regression test: wfx serve should not fail with asyncio error.

        This test verifies the fix for the issue where wfx serve failed with:
        ...
        proc = subprocess.Popen(  # noqa: S603
            ...
            text=True,
            env={**os.environ, "PRIMEAGENT_API_KEY": "test-key-12345"},  # pragma: allowlist secret
        )
            ...
            while time.time() - start_time < timeout:
                ...
                if exit_code is not None:
                    ...
                    if "asyncio.run() cannot be called from a running event loop" in output:
                        pytest.fail(f"Regression: wfx serve failed with asyncio error.\nOutput:\n{output}")
...
class TestAllStarterProjectsLoad:
    """Test that all starter projects can load without wfx-specific module errors."""
    ...
    def test_all_projects_load(self, starter_projects_path: Path):
        """Test all starter project JSONs can load without wfx-specific errors.

        Note: This test only fails on wfx-specific module errors (wfx.components.*),
        ...
        for json_file in json_files:
            try:
                ...
                error_str = str(e)
                # Only track wfx-specific errors, not external dependency errors
                if "No module named 'wfx." in error_str:
                    wfx_module_errors.append((json_file.name, str(e)))
                elif "resolve_component_path" in error_str:
                    wfx_module_errors.append((json_file.name, f"Storage error: {e}"))
                # External dependency errors (langchain_anthropic, etc.) are acceptable
                # as wfx is designed to work with minimal dependencies

        if wfx_module_errors:
            error_details = "\n".join([f"  {name}: {error}" for name, error in wfx_module_errors])
            pytest.fail(f"WFX module errors in starter projects:\n{error_details}")
